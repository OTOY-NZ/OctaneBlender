...
static void node_transform_oct_3d_init(bNodeTree *ntree, bNode *node)
{
  node->custom1 = OCT_ROT_YXZ;
  node_octane_transform_conversion_update(ntree, node);
} /* node_transform_oct_3d_init() */

void register_node_type_transform_oct_3d(void)
{
  static bNodeType ntype;

  if (ntype.type != SH_NODE_OCT_3D_TRN)
    node_type_base(
        &ntype, SH_NODE_OCT_3D_TRN, "3D Transform", NODE_CLASS_OCT_TRANSFORM, NODE_OPTIONS);
