...
static void node_type_render_aov_output_init(bNodeTree *ntree, bNode *node)
{
  node->oct_custom1 = 0;
  node->oct_custom2 = 0;
}
...
void register_node_type_render_aov_output(void)
{
  ...
  if (ntype.type != SH_NODE_OCT_RENDER_AOV_OUTPUT)
    node_type_base(&ntype,
                   SH_NODE_OCT_RENDER_AOV_OUTPUT,
                   "Render AOV output",
                   NODE_CLASS_OCT_COMPOSITE,
                   NODE_OPTIONS);
  node_type_socket_templates(&ntype, sh_node_in, sh_node_out);
  node_type_size(&ntype, 160, 160, 500);
  node_type_init(&ntype, node_type_render_aov_output_init);
