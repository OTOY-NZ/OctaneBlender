...
static blender::bke::bNodeSocketTemplate  sh_node_out[] = {{-1, ""}};

static void node_type_aov_output_group_init(bNodeTree *ntree, bNode *node)
{
  node->custom1 = 2;
...
static void node_shader_update_aov_output_group(bNodeTree *ntree, bNode *node)
{
  ...
  for (int i = 1; i <= 16; ++i) {
    ...
    char sock_name[64];
    sprintf(sock_name, "AOV output %d", i);
