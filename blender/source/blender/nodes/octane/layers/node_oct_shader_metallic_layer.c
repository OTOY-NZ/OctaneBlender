...
static void node_oct_metallic_layer_init(bNodeTree *ntree, bNode *node)
{
  node->custom1 = OCT_SHD_BRDF_MODEL_GGX;
} /* node_oct_metallic_layer_init() */
...
void register_node_type_sh_oct_metallic_layer(void)
{
  ...
  if (ntype.type != SH_NODE_OCT_METALLIC_LAYER)
    node_type_base(&ntype,
                   SH_NODE_OCT_METALLIC_LAYER,
                   "Octane Metallic Layer",
                   NODE_CLASS_OCT_LAYER,
                   NODE_OPTIONS);
  node_type_socket_templates(&ntype, sh_node_in, sh_node_out);
  node_type_size(&ntype, 160, 160, 500);
  node_type_init(&ntype, node_oct_metallic_layer_init);
