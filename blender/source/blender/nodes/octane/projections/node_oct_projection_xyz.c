...
static void node_projection_oct_xyz_init(bNodeTree *ntree, bNode *node)
{
  node->custom1 = OCT_POSITION_OBJECT;
  node_octane_projection_conversion_update(ntree, node);
} /* node_projection_oct_xyz_init() */
...
void register_node_type_projection_oct_xyz(void)
{
  ...
  node_type_init(&ntype, node_projection_oct_xyz_init);
