...
static void node_oct_init_metal_mat(bNodeTree *ntree, bNode *node)
{
...
void register_node_type_sh_oct_metal_mat(void)
{
  ...
  if (ntype.type != SH_NODE_OCT_METAL_MAT)
    node_type_base(&ntype,
                   SH_NODE_OCT_METAL_MAT,
                   "Metal Material",
                   NODE_CLASS_OCT_SHADER,
                   /* NODE_PREVIEW | */ NODE_OPTIONS);  //, 0);
  node_type_socket_templates(&ntype, sh_node_in, sh_node_out);
  node_type_size(&ntype, 160, 160, 500);
  node_type_init(&ntype, node_oct_init_metal_mat);
