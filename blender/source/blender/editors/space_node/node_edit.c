...
void ED_node_shader_default(const bContext *C, ID *id)
{
  ...
  if (GS(id->name) == ID_MA) {
    ...
    if (GS(id->name) == ID_WO) {
      ...
      if (is_octane_engine) {
        shader = nodeAddStaticNode(NULL, ntree, SH_NODE_OCT_TEXTURE_ENVIRONMENT);
        output = nodeAddStaticNode(NULL, ntree, SH_NODE_OUTPUT_WORLD);
        nodeAddLink(ntree,
                    shader,
                    nodeFindSocket(shader, SOCK_OUT, "OutEnv"),
                    output,
                    nodeFindSocket(output, SOCK_IN, "Octane Environment"));
      }
      else {
      ...
      const char *output_name = "OutMat";

      if (is_octane_engine) {
        int shader_type;
        if (light->type == LA_AREA || light->type == LA_SPHERE || light->type == LA_MESH) {
          shader_type = SH_NODE_OCT_DIFFUSE_MAT;
        }
        else if (light->type == LA_SUN) {
          shader_type = SH_NODE_OCT_TOON_DIRECTION_LIGHT;
          output_name = "OutLight";
        }
        else if (light->type == LA_LOCAL) {
          shader_type = SH_NODE_OCT_TOON_POINT_LIGHT;
          output_name = "OutLight";
        }
        shader = nodeAddStaticNode(NULL, ntree, shader_type);
        output = nodeAddStaticNode(NULL, ntree, SH_NODE_OUTPUT_LIGHT);
        nodeAddLink(ntree,
                    shader,
                    nodeFindSocket(shader, SOCK_OUT, output_name),
                    output,
                    nodeFindSocket(output, SOCK_IN, "Surface"));
        if (shader_type == SH_NODE_OCT_DIFFUSE_MAT) {
          bNode *in_emission = nodeAddStaticNode(NULL, ntree, SH_NODE_OCT_TEXTURE_EMI);
          nodeAddLink(ntree,
                      in_emission,
                      nodeFindSocket(in_emission, SOCK_OUT, "OutEmission"),
                      shader,
                      nodeFindSocket(shader, SOCK_IN, "Emission"));
          in_emission->locx = -300.f;
          in_emission->locy = 300.f;
        }
...
void ED_node_texture_default(const bContext *C, Tex *tex)
{
  ...
  bNode *in = nodeAddStaticNode(C, tex->nodetree, is_octane_engine ? SH_NODE_OCT_IMAGE_TEX : TEX_NODE_CHECKER);
  in->locx = 10.0f;
  in->locy = 300.0f;
...
static StructRNA *resolve_script_node_type(bContext *C)
{
  Scene *scene = CTX_data_scene(C);
  /* see if we have a shader script node in context */
  if (scene && strcmp(scene->r.engine, "octane") == 0) {
    PointerRNA ptr = CTX_data_pointer_get(C, "node");
    if (ptr.data) {
      if (RNA_struct_is_a(ptr.type, &RNA_ShaderNodeOctOSLTex)) {
        return &RNA_ShaderNodeOctOSLTex;
      }
      else if (RNA_struct_is_a(ptr.type, &RNA_ShaderNodeOctOSLCamera)) {
        return &RNA_ShaderNodeOctOSLCamera;
      }
      else if (RNA_struct_is_a(ptr.type, &RNA_ShaderNodeOctOSLBakingCamera)) {
        return &RNA_ShaderNodeOctOSLBakingCamera;
      }
      else if (RNA_struct_is_a(ptr.type, &RNA_ShaderNodeOctOSLProjection)) {
        return &RNA_ShaderNodeOctOSLProjection;
      }
      else if (RNA_struct_is_a(ptr.type, &RNA_ShaderNodeOctVectron)) {
        return &RNA_ShaderNodeOctVectron;
      }
    }
  }
  return &RNA_ShaderNodeScript;
}
...
static bool node_shader_script_update_poll(bContext *C)
{
  ...
  /* see if we have a shader script node in context */
  bNode *node = CTX_data_pointer_get_type(C, "node", resolve_script_node_type(C)).data;

  if (!node && snode && snode->edittree) {
    node = nodeGetActive(snode->edittree);
  }

  if (node && ELEM(node->type,
                   SH_NODE_SCRIPT,
                   SH_NODE_OCT_OSL_TEX,
                   SH_NODE_OCT_PROJECTION_OSL,
                   SH_NODE_OCT_OSL_CAMERA,
                   SH_NODE_OCT_OSL_BAKING_CAMERA,
                   SH_NODE_OCT_VECTRON)) {
...
static int node_shader_script_update_exec(bContext *C, wmOperator *op)
{
  ...
  Scene *scene = CTX_data_scene(C);
  SpaceNode *snode = CTX_wm_space_node(C);
  PointerRNA nodeptr = CTX_data_pointer_get_type(C, "node", resolve_script_node_type(C));
  bool found = false;
