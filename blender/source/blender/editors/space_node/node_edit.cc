...
static int node_clipboard_paste_exec(bContext *C, wmOperator *op)
{
  ...
  bool is_octane_aov = (ntree->type == NTREE_CUSTOM && strcmp(ntree->idname, "octane_render_aov_nodes") == 0);
  bool from_shader_to_octane_aov = (BKE_node_clipboard_get_type() == NTREE_SHADER &&
                                    is_octane_aov);

  if (BKE_node_clipboard_get_type() != ntree->type && !from_shader_to_octane_aov) {
...
static bool node_shader_script_update_poll(bContext *C)
{
  ...
  /* see if we have a shader script node in context */
  bNode *node = (bNode *)CTX_data_pointer_get_type(C, "node", resolve_script_node_type(C)).data;
