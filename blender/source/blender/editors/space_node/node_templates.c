...
static bool octane_ui_node_item_poll(bool is_octane_engine, const bNodeType *ntype)
{
  bool is_octane_node = STRPREFIX(ntype->idname, "ShaderNodeOct");
  return is_octane_engine == is_octane_node;
}
...
static void ui_node_menu_column(NodeLinkArg *arg, int nclass, const char *cname)
{
  ...
  NODE_TYPES_BEGIN (ntype) {
    ...
    if (!octane_ui_node_item_poll(is_octane_engine, ntype)) {
      continue;
    }
