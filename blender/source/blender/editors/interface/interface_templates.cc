                                     ...
                                     bool use_octane_format)
{
  ...
  UI_block_align_begin(block);
  if (use_octane_format) {
    uiItemR(row, &ptr, "octane_interpolation_type", 0, "", ICON_NONE);
  }
  else {
...
void uiTemplateColorRamp(
    uiLayout *layout, PointerRNA *ptr, const char *propname, bool expand, bool use_octane_format)
{
  PropertyRNA *prop = RNA_struct_find_property(ptr, propname);
  ...
  colorband_buttons_layout(layout, block, static_cast<ColorBand *>(cptr.data), &rect, cb, expand, use_octane_format);
