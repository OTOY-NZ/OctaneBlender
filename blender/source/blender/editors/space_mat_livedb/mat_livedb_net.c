...
/* ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 */
int mat_livedb_connect(const char *address)
{
  int cur_socket;
  struct hostent *host;
...
/* ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 */
void mat_livedb_disconnect(int _socket)
{
#ifndef WIN32
  close(_socket);
#else
...
static int mat_livedb_request(int _socket, uint64_t _type)
{
  ...
  *(uint64_t *)(buffer + sizeof(uint64_t)) = 0;
  ...
  if (send(_socket, (const char *)buffer, (unsigned int)sizeof(uint64_t) * 2, 0) !=
      (sizeof(uint64_t) * 2))
    return 0;
  else
    return 1;
...
int mat_livedb_request_material_new(int _socket)
{
  int32_t mat_id = 0;

  char buffer[sizeof(uint64_t) * 2 + sizeof(int32_t) + sizeof(char) * 1024];

  *(uint64_t *)buffer = GET_LDB_MAT_NEW;
  char *localdb_path = (char *)(buffer + sizeof(uint64_t) * 2);
  int path_str_size = 0;
  while (G.octane_localdb_path[path_str_size] != '\0') {
    localdb_path[path_str_size + 1] = G.octane_localdb_path[path_str_size];
    path_str_size++;
  }
  localdb_path[0] = path_str_size;
  if (path_str_size > 0) {
    localdb_path[path_str_size] = '\0';
  }
  else {
    localdb_path[1] = '\0';
  }

  unsigned int path_size = sizeof(char) * (path_str_size + 2);

  unsigned int size = sizeof(uint64_t) * 2 + path_size;

  *(uint64_t *)(buffer + sizeof(uint64_t)) = path_size;

  if (send(_socket, (const char *)buffer, size, 0) != size)
    return 0;
  else
    return 1;
} /* mat_livedb_request_material_new() */

/* ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 */
/* Get the reply from LiveDB. Returns dynamically allocated buffer. Responsibility of caller to
 * free it. */
/* ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 */
...
void *mat_livedb_receive_new(int _socket, uint64_t _type, uint32_t *buf_size, bool *is_close)
{
  uint64_t header[2];
  char *buffer;
  uint64_t type;
  ...
  if (recv(_socket, (char *)header, sizeof(uint64_t) * 2, MSG_WAITALL) != sizeof(uint64_t) * 2)
    ...
    type = header[0];
    *is_close = (type == GET_LDB_MAT_END);
    if (type < GET_LDB_MAT_NEW || type > GET_LDB_MAT_END) {
      *buf_size = 0;
      return 0;
    }
    else
...
/* ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 */
unsigned char *mat_livedb_get_mat_preview(const char *address,
                                          int32_t id,
                                          unsigned int *width,
                                          unsigned int *height,
