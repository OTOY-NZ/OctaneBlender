    ...
    switch (GS(id->name)) {
      ...
      case ID_NT: {
        ...
        for (node = static_cast<bNode *>(ntree->nodes.first); node; node = node->next) {
          ...
          if (ELEM(node->type,
                   SH_NODE_OCT_GRADIENT_TEX,
                   SH_NODE_OCT_VOLUME_RAMP_TEX,
                   SH_NODE_OCT_TOON_RAMP_TEX)) {
            BKE_ntree_update_tag_node_property(ntree, node);
...
static void rna_def_color_ramp(BlenderRNA *brna)
{
  ...
  static EnumPropertyItem octane_interpolation_type_items[] = {
      {OCT_INTERPOLATION_LINEAR, "OCTANE_INTERPOLATION_LINEAR", 0, "Linear", ""},
      {OCT_INTERPOLATION_CONSTANT, "OCTANE_INTERPOLATION_CONSTANT", 0, "Constant", ""},
      {OCT_INTERPOLATION_CUBIC, "OCTANE_INTERPOLATION_CUBIC", 0, "Cubic", ""},
  ...
  prop = RNA_def_property(srna, "octane_interpolation_type", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, NULL, "ipotype_octane");
  RNA_def_property_enum_items(prop, octane_interpolation_type_items);
  RNA_def_property_ui_text(prop, "Interpolation", "Determines how colours are blended");
  RNA_def_property_update(prop, 0, "rna_ColorRamp_update");
