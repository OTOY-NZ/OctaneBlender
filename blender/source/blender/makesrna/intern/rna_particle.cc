...
static void rna_def_particle_settings(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "octane_root_width", PROP_FLOAT, PROP_DISTANCE);
  RNA_def_property_float_sdna(prop, NULL, "octane_root_width");
  //RNA_def_property_float_default(prop, 0.001f);
  RNA_def_property_range(prop, 0.0f, 1000.0f);
  RNA_def_property_ui_range(prop, 0.0f, 1000.0f, 0.1, 3);
  RNA_def_property_ui_text(prop, "Root thickness", "Hair thickness at root");
  RNA_def_property_update(
      prop, 0, "rna_Particle_redo"); /* TODO: Only need to tell the render engine to update. */

  prop = RNA_def_property(srna, "octane_tip_width", PROP_FLOAT, PROP_DISTANCE);
  RNA_def_property_float_sdna(prop, NULL, "octane_tip_width");
  RNA_def_property_float_default(prop, 0.001f);
  RNA_def_property_range(prop, 0.0f, 1000.0f);
  RNA_def_property_ui_range(prop, 0.0f, 1000.0f, 0.1, 3);
  RNA_def_property_ui_text(prop, "Tip thickness", "Hair thickness at tip");
  RNA_def_property_update(
      prop, 0, "rna_Particle_redo"); /* TODO: Only need to tell the render engine to update. */

  prop = RNA_def_property(srna, "octane_min_curvature", PROP_FLOAT, PROP_DISTANCE);
  RNA_def_property_float_sdna(prop, NULL, "octane_min_curvature");
  RNA_def_property_float_default(prop, 0.0001f);
  RNA_def_property_range(prop, 0.0f, 180.0f);
  RNA_def_property_ui_range(prop, 0.0f, 180.0f, 0.1, 3);
  RNA_def_property_ui_text(prop,
                           "Minimal curvature (deg.)",
                           "Hair points having angle deviation from previous point less than this "
                           "value will be skipped");
  RNA_def_property_update(
      prop, 0, "rna_Particle_redo"); /* TODO: Only need to tell the render engine to update. */

  prop = RNA_def_property(srna, "octane_w_min", PROP_FLOAT, PROP_DISTANCE);
  RNA_def_property_float_sdna(prop, NULL, "octane_w_min");
  RNA_def_property_float_default(prop, 0.0f);
  RNA_def_property_range(prop, 0.0f, 1.0f);
  RNA_def_property_ui_range(prop, 0.0f, 1.0f, 0.1, 2);
  RNA_def_property_ui_text(prop,
                           "Min. W",
                           "W coordinate of the root of a hair: it represents a position on a "
                           "color gradient for roots of all hairs of this particle system");
  RNA_def_property_update(
      prop, 0, "rna_Particle_redo"); /* TODO: Only need to tell the render engine to update. */

  prop = RNA_def_property(srna, "octane_w_max", PROP_FLOAT, PROP_DISTANCE);
  RNA_def_property_float_sdna(prop, NULL, "octane_w_max");
  RNA_def_property_float_default(prop, 1.0f);
  RNA_def_property_range(prop, 0.0f, 1.0f);
  RNA_def_property_ui_range(prop, 0.0f, 1.0f, 0.1, 2);
  RNA_def_property_ui_text(prop,
                           "Max. W",
                           "W coordinate of the tip of a hair: it represents a position on a "
