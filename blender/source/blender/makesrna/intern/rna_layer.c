...
void RNA_def_view_layer(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "use_octane_render_layers", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_ui_text(
      prop, "Use Octane Render Layers", "Render current layers with octane render layers system");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, NULL);

  prop = RNA_def_property(srna, "octane_render_layer_active_id", PROP_INT, PROP_UNSIGNED);
  RNA_def_property_ui_text(
      prop, "Active layer ID", "ID of active render layer. Not applicable for multiple layers");
  RNA_def_property_range(prop, 1, 255);
  RNA_def_property_int_default(prop, 1);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, NULL);

  prop = RNA_def_property(srna, "octane_render_layers_mode", PROP_ENUM, PROP_NONE);
  RNA_def_property_ui_text(prop, "Mode", "Render mode");
  static EnumPropertyItem octane_render_layers_modes[] = {
      {OCT_RENDER_LAYER_MODE_NORMAL,
       "OCT_RENDER_LAYER_MODE_NORMAL",
       0,
       "Normal",
       "The beauty passes contain the active layer only and the render layer passes (shadows,"
       " reflections...) record the side-effects of the active render layer for those "
       "samples/pixels"
       " that are not obstructed by the active render layer"},
      {OCT_RENDER_LAYER_MODE_HIDE_INACTIVE_LAYERS,
       "OCT_RENDER_LAYER_MODE_HIDE_INACTIVE_LAYERS",
       0,
      ...
      {OCT_RENDER_LAYER_MODE_ONLY_SIDE_EFFECTS,
       "OCT_RENDER_LAYER_MODE_ONLY_SIDE_EFFECTS",
       0,
       "Only side effects",
       "The active layer will be made invisible and the render layer passes (shadows, "
       "reflections...)"
       ...
       "those"},
      {OCT_RENDER_LAYER_MODE_HIDE_FROM_CAMERA,
       "OCT_RENDER_LAYER_MODE_HIDE_FROM_CAMERA",
       0,
       "Hide from camera",
       "All geometry that is not on an active layer"
       " will be made invisible. But side effects(shadows, reflections...)will be recorded in the "
       "render layer passes"},
      {0, NULL, 0, NULL, NULL}};
  RNA_def_property_enum_items(prop, octane_render_layers_modes);
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, NULL);

  prop = RNA_def_property(srna, "octane_render_layers_invert", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_ui_text(prop,
                           "Invert",
                           "All the non-active render layers become the active render layer and "
                           "the active render layer becomes inactive");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, NULL);
