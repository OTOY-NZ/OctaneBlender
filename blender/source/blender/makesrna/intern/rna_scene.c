...
const EnumPropertyItem rna_enum_octane_image_save_mode_items[] = {
    ...
    {OCT_IMAGE_SAVE_MODE_MULTILAYER,
     ...
     "Export multilayer EXR"},
    {OCT_IMAGE_SAVE_MODE_DEEP_EXR, "DEEP_EXR", 0, "Export deep EXR", "Export deep EXR"},
    {0, NULL, 0, NULL, NULL},
};
...
const EnumPropertyItem rna_enum_octane_image_save_format_items[] = {
    {OCT_IMAGE_SAVE_FORMAT_PNG_8,
     "OCT_IMAGE_SAVE_FORMAT_PNG_8",
     0,
     "PNG(8-bit, integer)",
     "PNG(8-bit, integer)"},
    {OCT_IMAGE_SAVE_FORMAT_PNG_16,
     "OCT_IMAGE_SAVE_FORMAT_PNG_16",
     0,
     "PNG(16-bit, integer)",
     "PNG(16-bit, integer)"},
    {OCT_IMAGE_SAVE_FORMAT_EXR_16,
     "OCT_IMAGE_SAVE_FORMAT_EXR_16",
     0,
     "EXR(16-bit, floating point)",
     "EXR(16-bit, floating point)"},
    {OCT_IMAGE_SAVE_FORMAT_EXR_32,
     "OCT_IMAGE_SAVE_FORMAT_EXR_32",
     0,
...
void rna_def_octane_view_layer_common(StructRNA *srna, const bool scene)
{
  PropertyRNA *prop;
  prop = RNA_def_property(srna, "octane_aov_out_number", PROP_INT, PROP_UNSIGNED);
  RNA_def_property_int_sdna(prop, NULL, "octane_aov_out_number");
  RNA_def_property_range(prop, 0, 16);
  RNA_def_property_ui_text(prop, "AOV Outputs Number", "Number of AOV outputs");
  if (scene) {
    RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, "rna_ViewLayer_pass_update");
  }
  else {
...
#  define ADD_OCTANE_PASS_PROP(PROP, PASS_CATEGORY, PASS_TYPE, NAME, DESCRIPTION) \
    do { \
      prop = RNA_def_property(srna, PROP, PROP_BOOLEAN, PROP_NONE); \
      RNA_def_property_boolean_sdna(prop, NULL, #PASS_CATEGORY, PASS_TYPE); \
  ...
  ADD_OCTANE_PASS_PROP("use_pass_oct_beauty",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_BEAUTY,
                       "Beauty",
                       "Make the beauty pass raw render passes by dividing out the color of the "
                       "BxDF of the surface hit by the camera ray");
  ADD_OCTANE_PASS_PROP("use_pass_oct_emitters",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_EMITTERS,
                       "Emitters",
                       "Contains all samples where the camera ray hits an emitter");
  ADD_OCTANE_PASS_PROP("use_pass_oct_env",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_ENVIRONMENT,
                       "Environment",
                       "Contains the environment of the scene");
  ADD_OCTANE_PASS_PROP(
      "use_pass_oct_diff",
      oct_passflags[OCTANE_BEAUTY_PASS],
      OCT_SCE_PASS_DIFFUSE,
      "Diff",
      "Contains all samples where a diffuse material is lit either directly or indirectly");
  ADD_OCTANE_PASS_PROP("use_pass_oct_diff_dir",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_DIFFUSE_DIRECT,
                       "DiffDir",
                       "Contains all samples where a diffuse material is directly lit");
  ADD_OCTANE_PASS_PROP("use_pass_oct_diff_indir",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_DIFFUSE_INDIRECT,
                       "DiffIndir",
                       "Contains all samples where a diffuse material is indirectly lit (GI)");
  ADD_OCTANE_PASS_PROP("use_pass_oct_diff_filter",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_DIFFUSE_FILTER,
                       "DiffFilter",
                       "The diffuse texture color of the diffuse and glossy material");
  ADD_OCTANE_PASS_PROP("use_pass_oct_reflect",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_REFLECTION,
                       "Reflect",
                       "Contains all samples where either direct or indirect light is "
                       "reflected.directly or indirectly");
  ADD_OCTANE_PASS_PROP("use_pass_oct_reflect_dir",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_REFLECTION_DIRECT,
                       "ReflectDir",
                       "Contains all samples where direct light is reflected");
  ADD_OCTANE_PASS_PROP("use_pass_oct_reflect_indir",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_REFLECTION_INDIRECT,
                       "ReflectIndir",
                       "Contains all samples where indirect light is reflected");
  ADD_OCTANE_PASS_PROP("use_pass_oct_reflect_filter",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_REFLECTION_FILTER,
                       "ReflectFilter",
                       "The reflection texture color of the specular and glossy material");
  ADD_OCTANE_PASS_PROP("use_pass_oct_refract",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_REFRACTION,
                       "Refract",
                       "Contains all samples where the camera ray was refracted by a specular "
                       "material on the first bounce");
  ADD_OCTANE_PASS_PROP("use_pass_oct_refract_filter",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_REFRACTION_FILTER,
                       "RefractFilter",
                       "The refraction texture color of the specular material");
  ADD_OCTANE_PASS_PROP("use_pass_oct_transm",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_TRANSMISSION,
                       "Transm",
                       "Contains all samples where the camera ray is transmitted by a diffuse "
                       "material on the first bounce");
  ADD_OCTANE_PASS_PROP("use_pass_oct_transm_filter",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_TRANSMISSION_FILTER,
                       "TransmFilter",
                       "The transmission texture color of the diffuse material");
  ADD_OCTANE_PASS_PROP("use_pass_oct_sss",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_SSS,
                       "SSS",
  ...
  ADD_OCTANE_PASS_PROP(
      ...
      OCT_SCE_PASS_SHADOW,
      "Shadow",
      "Contains all direct light shadows that are calculated on the first path bounce. This "
      "includes sun light, but excludes sky light or texture environment if importance sampling "
      "is disabled or the texture environment doesn't consist of an image");
  ADD_OCTANE_PASS_PROP("use_pass_oct_irradiance",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_IRRADIANCE,
                       "Irradiance",
                       "Contains the irradiance on the surface");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_dir",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_LIGHT_DIRECTION,
                       "LightDir",
                       "Estimates the dominant direction from where most of the light is coming");
  ADD_OCTANE_PASS_PROP("use_pass_oct_volume",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_VOLUME,
                       "Volume",
                       "Contains all samples that scattered in a volume");
  ADD_OCTANE_PASS_PROP(
      "use_pass_oct_vol_mask",
      oct_passflags[OCTANE_BEAUTY_PASS],
      OCT_SCE_PASS_VOLUME_MASK,
      "VolMask",
      "Contains absorption color and the contribution amount of a volume sample. This is a "
  ...
  ADD_OCTANE_PASS_PROP("use_pass_oct_vol_emission",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_VOLUME_EMISSION,
                       "VolEmission",
                       "Contains all samples where the camera ray hit a volume emitter");
  ADD_OCTANE_PASS_PROP("use_pass_oct_vol_z_front",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_VOLUME_Z_FRONT,
                       "VolZFront",
                       "Contains the front depth of all volume samples");
  ADD_OCTANE_PASS_PROP("use_pass_oct_vol_z_back",
                       oct_passflags[OCTANE_BEAUTY_PASS],
                       OCT_SCE_PASS_VOLUME_Z_BACK,
                       "VolZBack",
  ...
  ADD_OCTANE_PASS_PROP(
      ...
      OCT_SCE_PASS_NOISE,
      "Noise",
      "Contains the noise estimate value. Green color if the noise estimate is lesser than the "
      "threshold. Will be black if adaptive sampling is disabled");

  ADD_OCTANE_PASS_PROP("use_pass_oct_denoise_beauty",
                       oct_passflags[OCTANE_DENOISE_PASS],
                       OCT_SCE_PASS_DENOISER_BEAUTY,
                       "DenoiserBeauty",
                       "");
  ADD_OCTANE_PASS_PROP("use_pass_oct_denoise_diff_dir",
                       oct_passflags[OCTANE_DENOISE_PASS],
                       OCT_SCE_PASS_DENOISER_DIFF_DIR,
                       "DenoiserDiffDir",
                       "Contains the denoised result of diffuse direct render passs");
  ADD_OCTANE_PASS_PROP("use_pass_oct_denoise_diff_indir",
                       oct_passflags[OCTANE_DENOISE_PASS],
                       OCT_SCE_PASS_DENOISER_DIFF_INDIR,
                       "DenoiserDiffIndir",
                       "Contains the denoised result of diffuse indirect render pass");
  ADD_OCTANE_PASS_PROP("use_pass_oct_denoise_reflect_dir",
                       oct_passflags[OCTANE_DENOISE_PASS],
                       OCT_SCE_PASS_DENOISER_REFLECTION_DIR,
                       "DenoiserReflectDir",
                       "Contains the denoised result of reflection direct render pass");
  ADD_OCTANE_PASS_PROP("use_pass_oct_denoise_reflect_indir",
                       oct_passflags[OCTANE_DENOISE_PASS],
                       OCT_SCE_PASS_DENOISER_REFLECTION_INDIR,
                       "DenoiserReflectIndir",
                       "Contains the denoised result of reflection indirect render pass");
  ADD_OCTANE_PASS_PROP("use_pass_oct_denoise_emission",
                       oct_passflags[OCTANE_DENOISE_PASS],
                       OCT_SCE_PASS_DENOISER_EMISSION,
                       "DenoiserEmission",
                       "Contains the denoised result of emission render pass");
  ADD_OCTANE_PASS_PROP(
      "use_pass_oct_denoise_remainder",
      oct_passflags[OCTANE_DENOISE_PASS],
      OCT_SCE_PASS_DENOISER_REMAINDER,
      "DenoiserRemainder",
      "Contains the denoised result of transmission and subsurface render passes");
  ADD_OCTANE_PASS_PROP("use_pass_oct_denoise_vol",
                       oct_passflags[OCTANE_DENOISE_PASS],
                       OCT_SCE_PASS_DENOISER_VOLUME,
                       "DenoiserVolume",
                       "Contains the denoised result of volume render pass");
  ADD_OCTANE_PASS_PROP("use_pass_oct_denoise_vol_emission",
                       oct_passflags[OCTANE_DENOISE_PASS],
                       OCT_SCE_PASS_DENOISER_VOLUME_EMISSION,
                       "DenoiserVolumeEmission",
                       "Contains the denoised result of volume render pass");

  ADD_OCTANE_PASS_PROP("use_pass_oct_postprocess",
                       oct_passflags[OCTANE_POST_PROCESSING_PASS],
                       OCT_SCE_PASS_POSTPROCESS,
                       "PostProcess",
                       "Contains the post-processing applied to the beauty pass. When enabled, no "
                       "post-processing is applied to the beauty pass itself");

  ADD_OCTANE_PASS_PROP(
      "use_pass_oct_layer_shadows",
      oct_passflags[OCTANE_LAYER_PASS],
      OCT_SCE_PASS_LAYER_SHADOWS,
      "LayerShadows",
      "Contains shadows cast by objects in the active render layer on objects in the other render "
      "layers. Combines black shadows and colored shadows in a single image");
  ADD_OCTANE_PASS_PROP(
      "use_pass_oct_layer_black_shadow",
      oct_passflags[OCTANE_LAYER_PASS],
      OCT_SCE_PASS_LAYER_BLACK_SHADOW,
      "LayerBlackShadow",
      "Contains shadows cast by opaque objects in the active render layer on objects in the other "
      "render layers. NOTE: this render pass doesn't work if the alpha channel is disabled");
  ADD_OCTANE_PASS_PROP("use_pass_oct_layer_reflections",
                       oct_passflags[OCTANE_LAYER_PASS],
                       OCT_SCE_PASS_LAYER_REFLECTIONS,
                       "LayerReflections",
                       "Contains light reflected by the objects in the active layer on the "
                       "objects in all the other layers");

  ADD_OCTANE_PASS_PROP("use_pass_oct_ambient_light",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_AMBIENT_LIGHT,
                       "AmbientLight",
                       "Captures the ambient light (sky and environment) in the scene");
  ADD_OCTANE_PASS_PROP("use_pass_oct_ambient_light_dir",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_AMBIENT_LIGHT_DIRECT,
                       "AmbientLightDir",
                       "Captures the indirect ambient light (sky and environment) in the scene");
  ADD_OCTANE_PASS_PROP("use_pass_oct_ambient_light_indir",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_AMBIENT_LIGHT_INDIRECT,
                       "AmbientLightIndir",
                       "Captures the indirect ambient light (sky and environment) in the scene");
  ADD_OCTANE_PASS_PROP("use_pass_oct_sunlight",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_SUNLIGHT,
                       "OctSunlight",
                       "Captures the sunlight in the scene");
  ADD_OCTANE_PASS_PROP("use_pass_oct_sunlight_dir",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_SUNLIGHT_DIRECT,
                       "SunLightDir",
                       "Captures the sunlight in the scene");
  ADD_OCTANE_PASS_PROP("use_pass_oct_sunlight_indir",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_SUNLIGHT_INDIRECT,
                       "SunLightIndir",
                       "Captures the sunlight in the scene");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_pass_1",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_PASS_1,
                       "LightPass1",
                       "Captures the light of the emitters with light pass ID 1");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_dir_pass_1",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_DIRECT_PASS_1,
                       "LightDirPass1",
                       "Captures the light of the emitters with light pass ID 1");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_indir_pass_1",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_INDIRECT_PASS_1,
                       "LightIndirPass1",
                       "Captures the light of the emitters with light pass ID 1");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_pass_2",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_PASS_2,
                       "LightPass2",
                       "Captures the light of the emitters with light pass ID 2");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_dir_pass_2",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_DIRECT_PASS_2,
                       "LightDirPass2",
                       "Captures the light of the emitters with light pass ID 2");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_indir_pass_2",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_INDIRECT_PASS_2,
                       "LightIndirPass2",
                       "Captures the light of the emitters with light pass ID 2");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_pass_3",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_PASS_3,
                       "LightPass3",
                       "Captures the light of the emitters with light pass ID 3");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_dir_pass_3",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_DIRECT_PASS_3,
                       "LightDirPass3",
                       "Captures the light of the emitters with light pass ID 3");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_indir_pass_3",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_INDIRECT_PASS_3,
                       "LightIndirPass3",
                       "Captures the light of the emitters with light pass ID 3");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_pass_4",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_PASS_4,
                       "LightPass4",
                       "Captures the light of the emitters with light pass ID 4");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_dir_pass_4",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_DIRECT_PASS_4,
                       "LightDirPass4",
                       "Captures the light of the emitters with light pass ID 4");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_indir_pass_4",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_INDIRECT_PASS_4,
                       "LightIndirPass4",
                       "Captures the light of the emitters with light pass ID 4");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_pass_5",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_PASS_5,
                       "LightPass5",
                       "Captures the light of the emitters with light pass ID 5");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_dir_pass_5",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_DIRECT_PASS_5,
                       "LightDirPass5",
                       "Captures the light of the emitters with light pass ID 5");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_indir_pass_5",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_INDIRECT_PASS_5,
                       "LightIndirPass5",
                       "Captures the light of the emitters with light pass ID 5");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_pass_6",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_PASS_6,
                       "LightPass6",
                       "Captures the light of the emitters with light pass ID 6");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_dir_pass_6",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_DIRECT_PASS_6,
                       "LightDirPass6",
                       "Captures the light of the emitters with light pass ID 6");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_indir_pass_6",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_INDIRECT_PASS_6,
                       "LightIndirPass6",
                       "Captures the light of the emitters with light pass ID 6");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_pass_7",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_PASS_7,
                       "LightPass7",
                       "Captures the light of the emitters with light pass ID 7");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_dir_pass_7",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_DIRECT_PASS_7,
                       "LightDirPass7",
                       "Captures the light of the emitters with light pass ID 7");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_indir_pass_7",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_INDIRECT_PASS_7,
                       "LightIndirPass7",
                       "Captures the light of the emitters with light pass ID 7");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_pass_8",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_PASS_8,
                       "LightPass8",
                       "Captures the light of the emitters with light pass ID 8");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_dir_pass_8",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_DIRECT_PASS_8,
                       "LightDirPass8",
                       "Captures the light of the emitters with light pass ID 8");
  ADD_OCTANE_PASS_PROP("use_pass_oct_light_indir_pass_8",
                       oct_passflags[OCTANE_LIGHTING_PASS],
                       OCT_SCE_PASS_LIGHT_INDIRECT_PASS_8,
                       "LightIndirPass8",
                       "Captures the light of the emitters with light pass ID 8");

  ADD_OCTANE_PASS_PROP("use_pass_oct_crypto_instance_id",
                       oct_passflags[OCTANE_CRYPTOMATTE_PASS],
                       OCT_SCE_PASS_CRYPTOMATTE_INSTANCE_ID,
                       "CryptoInstanceID",
                       "Cryptomatte channels for instance IDs");
  ADD_OCTANE_PASS_PROP("use_pass_oct_crypto_mat_node_name",
                       oct_passflags[OCTANE_CRYPTOMATTE_PASS],
                       OCT_SCE_PASS_CRYPTOMATTE_MATERIAL_NODE_NAME,
                       "CryptoMatNodeName",
                       "Cryptomatte channels using material node names");
  ADD_OCTANE_PASS_PROP("use_pass_oct_crypto_mat_node",
                       oct_passflags[OCTANE_CRYPTOMATTE_PASS],
                       OCT_SCE_PASS_CRYPTOMATTE_MATERIAL_NODE,
                       "CryptoMatNode",
                       "Cryptomatte channels using distinct material nodes");
  ADD_OCTANE_PASS_PROP("use_pass_oct_crypto_mat_pin_node",
                       oct_passflags[OCTANE_CRYPTOMATTE_PASS],
                       OCT_SCE_PASS_CRYPTOMATTE_MATERIAL_PIN_NODE,
                       "CryptoMatPinNode",
                       "Cryptomatte channels using material pin names");
  ADD_OCTANE_PASS_PROP("use_pass_oct_crypto_obj_node_name",
                       oct_passflags[OCTANE_CRYPTOMATTE_PASS],
                       OCT_SCE_PASS_CRYPTOMATTE_OBJECT_NODE_NAME,
                       "CryptoObjNodeName",
                       "Cryptomatte channels using object layer node names");
  ADD_OCTANE_PASS_PROP("use_pass_oct_crypto_obj_node",
                       oct_passflags[OCTANE_CRYPTOMATTE_PASS],
                       OCT_SCE_PASS_CRYPTOMATTE_OBJECT_NODE,
                       "CryptoObjNode",
                       "Cryptomatte channels using distinct object layer nodes");
  ADD_OCTANE_PASS_PROP("use_pass_oct_crypto_obj_pin_node",
                       oct_passflags[OCTANE_CRYPTOMATTE_PASS],
  ...
  ADD_OCTANE_PASS_PROP(
      ...
      OCT_SCE_PASS_GEOMETRIC_NORMAL,
      "GeoNormal",
      "Assigns a colour for the geometry normal at the position hit by the camera ray");
  ADD_OCTANE_PASS_PROP(
      "use_pass_oct_info_smooth_normal",
      oct_passflags[OCTANE_INFO_PASS],
      OCT_SCE_PASS_SMOOTH_NORMAL,
      "SmoothNormal",
      "Assigns a colour for the smooth normal at the position hit by the camera ray");
  ADD_OCTANE_PASS_PROP(
      "use_pass_oct_info_shading_normal",
      oct_passflags[OCTANE_INFO_PASS],
      OCT_SCE_PASS_SHADING_NORMAL,
      "ShadingNormal",
      "Assigns a colour for the shading normal at the position hit by the camera ray");
  ADD_OCTANE_PASS_PROP(
      "use_pass_oct_info_tangent_normal",
      oct_passflags[OCTANE_INFO_PASS],
      OCT_SCE_PASS_TANGENT_NORMAL,
      "TangentNormal",
      "Assigns a colour for the tangent (local) normal at the position hit by the camera ray");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_z_depth",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_Z_DEPTH,
                       "ZDepth",
                       "Assigns a gray value proportional to the camera ray hit distance");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_position",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_POSITION,
                       "Position",
                       "Assigns RGB values according the intersection point of the camera ray");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_uv",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_UV_COORDINATES,
                       "UV",
                       "Assigns RGB values according the geometry's texture coordinates");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_tex_tangent",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_TEXTURE_TANGENT,
                       "TexTangent",
                       "The tangent vector of U texture coordinates (Dp du)");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_motion_vector",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_MOTION_VECTOR,
                       "MotionVector",
                       "Renders the motion vectors as 2D vectors in screen space");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_mat_id",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_MATERIAL_ID,
                       "MatID",
                       "Assigns RGB values according the material mapped to the geometry");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_obj_id",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_OBJECT_ID,
                       "ObjID",
                       "Colours each distinct object in the scene with a colour based on it's ID");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_obj_layer_color",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_OBJECT_LAYER_COLOR,
                       "ObjLayerColor",
                       "The color specified in the object layer node");
  ADD_OCTANE_PASS_PROP(
      "use_pass_oct_info_baking_group_id",
      oct_passflags[OCTANE_INFO_PASS],
      OCT_SCE_PASS_BAKING_GROUP_ID,
      "BakingGroupID",
      "Colours each distinct baking group in the scene with a colour based on it's ID");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_light_pass_id",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_LIGHT_PASS_ID,
                       "LightPassID",
                       "Colours emitters based on their light pass ID");
  ADD_OCTANE_PASS_PROP(
      "use_pass_oct_info_render_layer_id",
      oct_passflags[OCTANE_INFO_PASS],
      OCT_SCE_PASS_RENDER_LAYER_ID,
      "RenderLayerID",
      "Colours objects on the same layer with the same color based on the render layer ID");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_render_layer_mask",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_RENDER_LAYER_MASK,
                       "RenderLayerMask",
                       "Mask for geometry on the active render layer");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_wireframe",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_WIREFRAME,
                       "Wireframe",
                       "Wireframe display of the geometry");
  ADD_OCTANE_PASS_PROP("use_pass_oct_info_ao",
                       oct_passflags[OCTANE_INFO_PASS],
                       OCT_SCE_PASS_AMBIENT_OCCLUSION,
                       "AO",
                       "Assigns a colour to the camera ray's hit point proportional to the amount "
                       "of occlusion by other geometry");

  ADD_OCTANE_PASS_PROP("use_pass_oct_mat_opacity",
                       oct_passflags[OCTANE_MATERIAL_PASS],
                       OCT_SCE_PASS_OPACITY,
                       "Opacity",
                       "Assigns a colour to the camera ray's hit point proportional to the "
                       "opacity of the geometry");
  ADD_OCTANE_PASS_PROP("use_pass_oct_mat_roughness",
                       oct_passflags[OCTANE_MATERIAL_PASS],
                       OCT_SCE_PASS_ROUGHNESS,
                       "Roughness",
                       "Material roughness at the camera ray's hit point");
  ADD_OCTANE_PASS_PROP("use_pass_oct_mat_ior",
                       oct_passflags[OCTANE_MATERIAL_PASS],
                       OCT_SCE_PASS_INDEX_OF_REFRACTION,
                       "IOR",
                       "Material index of refraction at the camera ray's hit point");
  ADD_OCTANE_PASS_PROP("use_pass_oct_mat_diff_filter_info",
                       oct_passflags[OCTANE_MATERIAL_PASS],
                       OCT_SCE_PASS_DIFFUSE_FILTER_INFO,
                       "DiffFilterInfo",
                       "The diffuse texture color of the diffuse and glossy material");
  ADD_OCTANE_PASS_PROP("use_pass_oct_mat_reflect_filter_info",
                       oct_passflags[OCTANE_MATERIAL_PASS],
                       OCT_SCE_PASS_REFLECTION_FILTER_INFO,
                       "ReflectFilterInfo",
                       "The reflection texture color of the specular and glossy material");
  ADD_OCTANE_PASS_PROP("use_pass_oct_mat_refract_filter_info",
                       oct_passflags[OCTANE_MATERIAL_PASS],
                       OCT_SCE_PASS_REFRACTION_FILTER_INFO,
                       "RefractFilterInfo",
                       "The refraction texture color of the specular material");
  ADD_OCTANE_PASS_PROP("use_pass_oct_mat_transm_filter_info",
                       oct_passflags[OCTANE_MATERIAL_PASS],
                       OCT_SCE_PASS_TRANSMISSION_FILTER_INFO,
                       "TransmFilterInfo",
                       "The transmission texture color of the diffuse material");

#  undef ADD_OCTANE_PASS_PROP

#  define ADD_OCTANE_PASS_PROP_HELPER(PROP, PASS_CATEGORY) \
    do { \
      prop = RNA_def_property(srna, PROP, PROP_INT, PROP_NONE); \
      RNA_def_property_int_sdna(prop, NULL, #PASS_CATEGORY); \
      RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, NULL); \
    } while (false)

  ADD_OCTANE_PASS_PROP_HELPER("oct_beauty_passflags", oct_passflags[OCTANE_BEAUTY_PASS]);
  ADD_OCTANE_PASS_PROP_HELPER("oct_denoise_passflags", oct_passflags[OCTANE_DENOISE_PASS]);
  ADD_OCTANE_PASS_PROP_HELPER("oct_postprocess_passflags",
                              oct_passflags[OCTANE_POST_PROCESSING_PASS]);
  ADD_OCTANE_PASS_PROP_HELPER("oct_layer_passflags", oct_passflags[OCTANE_LAYER_PASS]);
  ADD_OCTANE_PASS_PROP_HELPER("oct_lighting_passflags", oct_passflags[OCTANE_LIGHTING_PASS]);
  ADD_OCTANE_PASS_PROP_HELPER("oct_cryptomatte_passflags", oct_passflags[OCTANE_CRYPTOMATTE_PASS]);
  ADD_OCTANE_PASS_PROP_HELPER("oct_info_passflags", oct_passflags[OCTANE_INFO_PASS]);
  ADD_OCTANE_PASS_PROP_HELPER("oct_material_passflags", oct_passflags[OCTANE_MATERIAL_PASS]);

#  undef ADD_OCTANE_PASS_PROP_HELPER
}
...
void rna_def_view_layer_common(BlenderRNA *brna, StructRNA *srna, const bool scene)
{
  ...
  rna_def_octane_view_layer_common(srna, scene);
}
...
#  endif
  ...
  RNA_def_property_ui_text(
      prop, "Octane Export Mode", "Export image as separate files, multilayer EXR, or deep EXR");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, NULL);
  ...
  RNA_def_property_ui_text(
      prop,
      "Octane Prefix Tag",
      "Octane export prefix tag. If given, this name will be concatenated to the file name as "
      "prefix(to distinguish the Blender outputs and Octane export outputs)");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, NULL);

  prop = RNA_def_property(srna, "octane_export_post_tag", PROP_STRING, PROP_NONE);
  RNA_def_property_string_sdna(prop, NULL, "octane_export_post_tag");
  RNA_def_property_ui_text(
      prop,
      "Octane Postfix Tag",
      "Octane export postfix. If given, this name will be concatenated to the file name as "
      "postfix(use $OCTANE_PASS$ to label to octane pass name)");
  RNA_def_property_update(prop, NC_SCENE | ND_RENDER_OPTIONS, NULL);
