    ...
    def execute(self, context):
        ...
        if context.scene.render.use_shading_nodes:
            ...
            if context.scene.render.engine != 'octane':
                ...
                node_out.location = grid_location(2, 3)

                # Add Octane shader
                node_add_shader_1 = nodes.new(type='ShaderNodeOctVolumeMedium')
                node_add_shader_1.location = grid_location(1, 3)
                links.new(node_add_shader_1.outputs["OutTex"], node_out.inputs["Volume"])
