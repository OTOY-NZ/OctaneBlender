...
static int node_shader_gpu_oct_tex_image(GPUMaterial *mat, bNode *node, bNodeExecData *UNUSED(execdata), GPUNodeStack *in, GPUNodeStack *out) {
	int          ret;
    float        rgb[4];
    int          isdata;
    ...
    if(!tex) {
        node_oct_image_tex_init(0, node);
        tex   = node->storage;
    }
...
void register_node_type_tex_oct_image(void) {
	...
	if(ntype.type != SH_NODE_OCT_IMAGE_TEX) node_type_base(&ntype, SH_NODE_OCT_IMAGE_TEX, "Octane Image Tex", NODE_CLASS_OCT_TEXTURE, NODE_OPTIONS);
	...
	node_type_gpu(&ntype, node_shader_gpu_oct_tex_image);
