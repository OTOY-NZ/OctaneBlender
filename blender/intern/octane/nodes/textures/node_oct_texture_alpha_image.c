...
static void node_oct_alpha_image_tex_init(bNodeTree *ntree, bNode *node) {
	...
	tex->iuser.ok= 1;
	node->storage = tex;
} /* node_oct_alpha_image_tex_init() */

static int node_shader_gpu_oct_tex_alpha_image(GPUMaterial *mat, bNode *node, bNodeExecData *UNUSED(execdata), GPUNodeStack *in, GPUNodeStack *out) {
	int          ret;
    float        rgb[4];
    int          isdata;
    ...
    if(!tex) {
        node_oct_alpha_image_tex_init(0, node);
        tex   = node->storage;
    }
...
void register_node_type_tex_oct_alpha_image(void) {
	...
	if(ntype.type != SH_NODE_OCT_ALPHA_IMAGE_TEX) node_type_base(&ntype, SH_NODE_OCT_ALPHA_IMAGE_TEX, "Octane Alpha Image Tex", NODE_CLASS_OCT_TEXTURE, NODE_OPTIONS);
    node_type_compatibility(&ntype, NODE_NEW_SHADING);
	node_type_socket_templates(&ntype, sh_node_in, sh_node_out);
	node_type_size(&ntype, 200, 150, 250);
    node_type_init(&ntype, node_oct_alpha_image_tex_init);
	node_type_storage(&ntype, "NodeTexImage", node_free_standard_storage, node_copy_standard_storage);
	node_type_exec(&ntype, 0, 0, 0);
	node_type_gpu(&ntype, node_shader_gpu_oct_tex_alpha_image);
