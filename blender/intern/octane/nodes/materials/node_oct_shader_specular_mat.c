...
static int node_shader_gpu_oct_specular_mat(struct GPUMaterial *mat, struct bNode *UNUSED(node), struct bNodeExecData *UNUSED(execdata), struct GPUNodeStack *in, struct GPUNodeStack *out) {
    in[3].type = GPU_NONE;
	return GPU_stack_link(mat, "node_bsdf_glass", in, out, GPU_builtin(GPU_VIEW_NORMAL));
}
...
void register_node_type_sh_oct_specular_mat(void) {
    ...
    if(ntype.type != SH_NODE_OCT_SPECULAR_MAT) node_type_base(&ntype, SH_NODE_OCT_SPECULAR_MAT, "Octane Specular Material", NODE_CLASS_OCT_SHADER, /* NODE_PREVIEW | */ NODE_OPTIONS);
