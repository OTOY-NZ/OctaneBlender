...
def update_ocio_info_xml_request(ocio_config_file_path, ocio_use_other_config_file, ocio_use_automatic,
                                 ocio_intermediate_color_space_octane,
                                 octane_format_ocio_intermediate_color_space_ocio):
    from octane.core.client import OctaneBlender
    root_et = ElementTree.Element("updateOCIOInfo")
    ...
    if len(response):
        ...
        try:
            ...
            return [role_names, role_color_space_names, color_space_names, color_space_family_names, display_names,
                    display_view_names, look_names, [color_space_octane_value, color_space_ocio_name]]
...
def update_ocio_info(_self=None, _context=None):
    ...
    preferences = utility.get_preferences()
    ocio_intermediate_color_space_octane = \
        preferences.rna_type.properties['ocio_intermediate_color_space_octane'].enum_items[
            preferences.ocio_intermediate_color_space_octane].value
    ...
    if core.ENABLE_OCTANE_ADDON_CLIENT:
        results = update_ocio_info_xml_request(preferences.ocio_config_file_path,
                                               preferences.ocio_use_other_config_file, ocio_use_automatic,
                                               ocio_intermediate_color_space_octane,
                                               preferences.octane_format_ocio_intermediate_color_space_ocio)
    else:
        import _octane
        results = _octane.update_ocio_info(preferences.ocio_config_file_path, preferences.ocio_use_other_config_file,
                                           ocio_use_automatic, ocio_intermediate_color_space_octane,
                                           preferences.octane_format_ocio_intermediate_color_space_ocio)
    ...
    export_exr_default_name_map = {' Linear sRGB(default) ': 'Linear sRGB(default)', ' ACES2065-1 ': 'ACES2065-1',
                                   ' ACEScg ': 'ACEScg'}
    role_names = list(ocio_manager.raw_ocio_info[0])
    role_color_space_names = list(ocio_manager.raw_ocio_info[1])
    ui_role_names = list(map(lambda x, y: "[Role]" + x + ((" (" + y + ")") if len(y) else ""),
                             role_names, role_color_space_names))
    ...
    ui_color_space_names = list(map(lambda x, y: "[ColorSpace]" + x + ((" (" + y + ")") if len(y) else ""),
                                    color_space_names, color_space_family_names))
    ...
    set_container(preferences.ocio_intermediate_color_space_configs,
                  intermediate_default_names + ui_role_names + ui_color_space_names)
    if preferences.octane_format_ocio_intermediate_color_space_ocio == '' and not ocio_use_automatic:
        ...
        set_container(preferences.ocio_export_png_color_space_configs,
                      export_png_default_names + ui_role_names + ui_color_space_names)
        set_container(preferences.ocio_export_exr_color_space_configs,
                      export_exr_default_names + ui_role_names + ui_color_space_names)
    ...
    ui_ocio_view_names = list(map(lambda x, y: ((x + ": ") if len(x) else "") + y, display_names, display_view_names))
...
def update_ocio_intermediate_color_space_ocio(_self=None, _context=None):
    try:
        preferences = utility.get_preferences()
        preferences.octane_format_ocio_intermediate_color_space_ocio = OctaneOCIOManagement().ocio_config_map.get(
            preferences.ocio_intermediate_color_space_ocio, '')
    except Exception as e:
        logger.exception(e)
    update_ocio_info()


def update_ocio_view(self, _context):
...
def update_octane_export_ocio_params(self, _context):
    self.octane_export_ocio_color_space_name = OctaneOCIOManagement().ocio_config_map.get(
        self.gui_octane_export_ocio_color_space_name, '')
