...
class ObjectMaterialTagManager(object):
    ...
    @staticmethod
    def resolve_tag(material_cache, view_layer, _object):
        if _object is not None:
            ...
            if origin_object.material_slots is not None and len(origin_object.material_slots) > 0:
                ...
                for mat_slot in origin_object.material_slots:
                    mat = mat_slot.material if view_layer.material_override is None else view_layer.material_override
    ...
    def update_tag(self, material_cache, view_layer, _object):
        new_tag = self.resolve_tag(material_cache, view_layer, _object)
    ...
    def update_object_material_tags(self, material_cache, depsgraph, updated_object_names):
        for _object in depsgraph.scene.objects:
            ...
            if obj_blender_id in self.object_to_material_tag_map:
                is_updated, new_tag = self.update_tag(material_cache, depsgraph.view_layer, _object)
                if is_updated:
...
class ObjectCache(OctaneNodeCache):
        ...
        for dg_update in depsgraph.updates:
            ...
            if dg_update.is_updated_geometry:
                ...
                if dg_update_id_data is not None:
                    ...
                    if is_edit_mode:
                        # when an object is under edit mode, use its original.
                        # otherwise, the editing object data is always named as "Mesh".
                        dg_update_id_data = getattr(dg_update.id.original, "data", None)
    ...
    def is_object_data_updated_in_depsgraph(self, eval_object):
    ...
    def update_mesh_data(self, _depsgraph, origin_object, eval_object, mesh, octane_node, octane_property,
        ...
        if motion_time_offset == 0:
            ...
            # Auto smooth angle
            auto_smooth_angle = -1.0
            for b_mod in origin_object.modifiers:
                mod_name = b_mod.name
                if b_mod.type == 'NODES' and mod_name.startswith("Auto Smooth"):
                    auto_smooth_angle = 180.0
            # Loop triangles
    ...
    def update_mesh(self, object_data_name, depsgraph, eval_object, octane_property, octane_material_tag,
        ...
        if not need_full_update:
            return
        if octane_property.enable_mesh_volume:
        ...
        octane_node = self.get_octane_node(object_data_name, octane_node_type)
        # Materials
        ...
        octane_node.node.set_mesh_volume_attribute(octane_property.mesh_volume_sdf_voxel_size,
                                                   octane_property.mesh_volume_sdf_border_thickness_inside,
                                                   octane_property.mesh_volume_sdf_border_thickness_outside)
        # Geometry Data
    ...
    def update_light(self, object_data_name, scatter_node, depsgraph, eval_object, octane_property,
                     need_full_update, update_now):
        scene = depsgraph.scene
    ...
    def update_object(self, scene, depsgraph, eval_object, octane_scatter_node, update_now):
        ...
        # Is the object data updated in the depsgraph?
        # If so, make a full update of the object data
        need_update_object_data = (self.is_object_data_updated_in_depsgraph(eval_object)
                                   or getattr(octane_scatter_node, "is_newly_created", False))
        need_full_update_object_data = need_update_object_data
        ...
        # Update Object Data
        # Material tags
        _, octane_material_tag = self.object_material_tag_manager.update_tag(self.session.material_cache,
        ...
        if eval_object.type == "MESH":
            ...
            objectlayer_map_linked_mesh_name = self.update_light(object_data_name, octane_scatter_node, depsgraph,
    ...
    def custom_update(self, depsgraph, scene, view_layer, context=None, update_now=True):
        ...
        for instance_object in depsgraph.object_instances:
            ...
            if is_viewport:
                # Is this object a depsgraph update?
                need_sync |= eval_object_blender_id in self.changed_data_ids
                # Is this object's data a depsgraph update?
            ...
            if ob_octane.enable_octane_offset_transform:
                offset_matrix = utility.compose_matrix(ob_octane.octane_offset_translation,
                                                       ...
                                                       ob_octane.octane_offset_scale,
                                                       ob_octane.octane_offset_rotation_order)
