...
class OctaneVolumeMedium(OctaneBaseSocket):
    ...
    octane_default_node_type = consts.NodeType.NT_MED_VOLUME
...
class OctaneVolume(bpy.types.Node, OctaneBaseNode):
    ...
    a_volume_isovalue: FloatProperty(name="Volume isovalue", default=0.040000, update=OctaneBaseNode.update_node_tree, description="Isovalue used when converting between fog volumes and levelsets")
    a_border_width: FloatVectorProperty(name="Border width", default=(3.000000, 3.000000), size=2, update=OctaneBaseNode.update_node_tree, description="Amount of voxels that will be generated on either side of the surface (outside, inside) when loading a mesh or a fog volume as an SDF volume")
    ...
    a_voxel_size: FloatProperty(name="Voxel size", default=1.000000, update=OctaneBaseNode.update_node_tree, description="Dimension of one voxel")
