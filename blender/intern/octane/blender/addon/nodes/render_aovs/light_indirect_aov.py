...
class OctaneLightIndirectAOVSubType(OctaneBaseSocket):
    ...
    default_value: EnumProperty(default="Light ID 1", description="The ID of the indirect light AOV", items=items)
...
class OctaneLightIndirectAOV(bpy.types.Node, OctaneBaseNode):
    ...
    def init(self, context):
        self.inputs.new("OctaneLightIndirectAOVEnabled", OctaneLightIndirectAOVEnabled.bl_label)
        self.inputs.new("OctaneLightIndirectAOVSubType", OctaneLightIndirectAOVSubType.bl_label)
        self.outputs.new("OctaneRenderAOVsOutSocket", "Render AOVs out")


def register():
    register_class(OctaneLightIndirectAOVEnabled)
    register_class(OctaneLightIndirectAOVSubType)
    register_class(OctaneLightIndirectAOV)

def unregister():
    unregister_class(OctaneLightIndirectAOV)
    unregister_class(OctaneLightIndirectAOVSubType)
    unregister_class(OctaneLightIndirectAOVEnabled)
