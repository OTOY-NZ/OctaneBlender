...
class OctaneDiffuseFilterInfoAOV(bpy.types.Node, OctaneBaseNode):
    ...
    def init(self, context):
        self.inputs.new("OctaneDiffuseFilterInfoAOVEnabled", OctaneDiffuseFilterInfoAOVEnabled.bl_label)
        self.outputs.new("OctaneRenderAOVsOutSocket", "Render AOVs out")


def register():
    register_class(OctaneDiffuseFilterInfoAOVEnabled)
    register_class(OctaneDiffuseFilterInfoAOV)

def unregister():
    unregister_class(OctaneDiffuseFilterInfoAOV)
    unregister_class(OctaneDiffuseFilterInfoAOVEnabled)
