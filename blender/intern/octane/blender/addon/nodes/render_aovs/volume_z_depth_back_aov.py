...
class OctaneVolumeZDepthBackAOV(bpy.types.Node, OctaneBaseNode):
    ...
    octane_node_type: IntProperty(name="Octane Node Type", default=252)
    octane_socket_list: StringProperty(name="Socket List", default="Enabled;")
    octane_attribute_list: StringProperty(name="Attribute List", default="")
    bl_width_default = 160

    def init(self, context):
        self.inputs.new("OctaneVolumeZDepthBackAOVEnabled", OctaneVolumeZDepthBackAOVEnabled.bl_label)
        self.outputs.new("OctaneRenderAOVsOutSocket", "Render AOVs out")


def register():
    register_class(OctaneVolumeZDepthBackAOVEnabled)
    register_class(OctaneVolumeZDepthBackAOV)

def unregister():
    unregister_class(OctaneVolumeZDepthBackAOV)
    unregister_class(OctaneVolumeZDepthBackAOVEnabled)
