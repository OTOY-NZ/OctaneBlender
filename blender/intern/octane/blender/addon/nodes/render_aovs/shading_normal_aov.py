...
class OctaneShadingNormalAOVBump(OctaneBaseSocket):
    ...
    octane_pin_type: IntProperty(name="Octane Pin Type", default=1)
    octane_socket_type: IntProperty(name="Socket Type", default=1)
    default_value: BoolProperty(default=True, description="Take bump and normal mapping into account for shading normal")
...
class OctaneShadingNormalAOV(bpy.types.Node, OctaneBaseNode):
    ...
    def init(self, context):
        self.inputs.new("OctaneShadingNormalAOVEnabled", OctaneShadingNormalAOVEnabled.bl_label)
        self.inputs.new("OctaneShadingNormalAOVBump", OctaneShadingNormalAOVBump.bl_label)
        self.outputs.new("OctaneRenderAOVsOutSocket", "Render AOVs out")


def register():
    register_class(OctaneShadingNormalAOVEnabled)
    register_class(OctaneShadingNormalAOVBump)
    register_class(OctaneShadingNormalAOV)

def unregister():
    unregister_class(OctaneShadingNormalAOV)
    unregister_class(OctaneShadingNormalAOVBump)
    unregister_class(OctaneShadingNormalAOVEnabled)
