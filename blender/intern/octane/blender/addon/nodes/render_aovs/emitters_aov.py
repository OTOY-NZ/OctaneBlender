...
class OctaneEmittersAOV(bpy.types.Node, OctaneBaseNode):
    ...
    def init(self, context):
        self.inputs.new("OctaneEmittersAOVEnabled", OctaneEmittersAOVEnabled.bl_label)
        self.outputs.new("OctaneRenderAOVsOutSocket", "Render AOVs out")


def register():
    register_class(OctaneEmittersAOVEnabled)
    register_class(OctaneEmittersAOV)

def unregister():
    unregister_class(OctaneEmittersAOV)
    unregister_class(OctaneEmittersAOVEnabled)
