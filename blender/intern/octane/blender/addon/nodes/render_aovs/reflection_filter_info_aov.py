...
class OctaneReflectionFilterInfoAOV(bpy.types.Node, OctaneBaseNode):
    ...
    octane_node_type: IntProperty(name="Octane Node Type", default=226)
    octane_socket_list: StringProperty(name="Socket List", default="Enabled;")
    octane_attribute_list: StringProperty(name="Attribute List", default="")
    bl_width_default = 160

    def init(self, context):
        self.inputs.new("OctaneReflectionFilterInfoAOVEnabled", OctaneReflectionFilterInfoAOVEnabled.bl_label)
        self.outputs.new("OctaneRenderAOVsOutSocket", "Render AOVs out")


def register():
    register_class(OctaneReflectionFilterInfoAOVEnabled)
    register_class(OctaneReflectionFilterInfoAOV)

def unregister():
    unregister_class(OctaneReflectionFilterInfoAOV)
    unregister_class(OctaneReflectionFilterInfoAOVEnabled)
