...
class OctaneTransmissionAOV(bpy.types.Node, OctaneBaseNode):
    ...
    octane_node_type: IntProperty(name="Octane Node Type", default=244)
    octane_socket_list: StringProperty(name="Socket List", default="Enabled;")
    octane_attribute_list: StringProperty(name="Attribute List", default="")
    bl_width_default = 160

    def init(self, context):
        self.inputs.new("OctaneTransmissionAOVEnabled", OctaneTransmissionAOVEnabled.bl_label)
        self.outputs.new("OctaneRenderAOVsOutSocket", "Render AOVs out")


def register():
    register_class(OctaneTransmissionAOVEnabled)
    register_class(OctaneTransmissionAOV)

def unregister():
    unregister_class(OctaneTransmissionAOV)
    unregister_class(OctaneTransmissionAOVEnabled)
