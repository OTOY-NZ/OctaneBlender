...
class OctaneImagerDenoiser(OctaneBaseSocket):
    ...
    octane_pin_index=20
    octane_socket_type=consts.SocketType.ST_BOOL
    default_value: BoolProperty(default=False, update=OctaneBaseSocket.update_node_tree, description="Turns on the denoising feature. Depending on the chosen denoising method, it cleans up the main beauty AOV and a few other beauty AOVs (specifically with the Octane denoiser), and then saves the result in separate passes")
    octane_hide_value=False
    octane_min_version=4000001
    octane_end_version=4294967295
    octane_deprecated=False

class OctaneImagerDenoiserType(OctaneBaseSocket):
    bl_idname="OctaneImagerDenoiserType"
    bl_label="Denoiser"
    color=consts.OctanePinColor.Enum
    octane_default_node_type=consts.NodeType.NT_ENUM
    octane_default_node_name="OctaneEnumValue"
    octane_pin_id=consts.PinID.P_DENOISER_TYPE
    octane_pin_name="denoiserType"
    octane_pin_type=consts.PinType.PT_ENUM
    octane_pin_index=21
    octane_socket_type=consts.SocketType.ST_ENUM
    items = [
        ("Octane AI denoiser", "Octane AI denoiser", "", 0),
        ("Open Image Denoise", "Open Image Denoise", "", 1),
    ]
    default_value: EnumProperty(default="Octane AI denoiser", update=OctaneBaseSocket.update_node_tree, description="The denoising method to utilize for reducing noise", items=items)
    octane_hide_value=False
...
class OctaneImagerDenoiseVolume(OctaneBaseSocket):
    ...
    octane_pin_index=22
    octane_socket_type=consts.SocketType.ST_BOOL
    default_value: BoolProperty(default=False, update=OctaneBaseSocket.update_node_tree, description="Whether the Octane spectral AI denoiser will denoise volumes in the scene")
    octane_hide_value=False
    octane_min_version=4000011
    octane_end_version=4294967295
    octane_deprecated=False

class OctaneImagerDenoisePrefilter(OctaneBaseSocket):
    bl_idname="OctaneImagerDenoisePrefilter"
    bl_label="Prefilter auxiliary AOVs"
    color=consts.OctanePinColor.Bool
    octane_default_node_type=consts.NodeType.NT_BOOL
    octane_default_node_name="OctaneBoolValue"
    octane_pin_id=consts.PinID.P_DENOISE_PREFILTER
    octane_pin_name="denoisePrefilter"
    octane_pin_type=consts.PinType.PT_BOOL
    octane_pin_index=23
    octane_socket_type=consts.SocketType.ST_BOOL
...
class OctaneImagerDenoiserOnce(OctaneBaseSocket):
    ...
    octane_pin_index=24
    octane_socket_type=consts.SocketType.ST_BOOL
    default_value: BoolProperty(default=True, update=OctaneBaseSocket.update_node_tree, description="If enabled, beauty AOVs will be denoised only once at the end of a render. This option should be disabled while rendering with an interactive region")
    octane_hide_value=False
    octane_min_version=4000001
    octane_end_version=4294967295
    octane_deprecated=False
...
class OctaneImagerGroupDenoiser(OctaneGroupTitleSocket):
    bl_idname="OctaneImagerGroupDenoiser"
    bl_label="[OctaneGroupTitle]Denoiser"
    octane_group_sockets: StringProperty(name="Group Sockets", default="Enable denoising;Denoiser;Denoise volumes;Prefilter auxiliary AOVs;Denoise on completion;Min. denoiser samples;Max. denoiser interval;Blend;")
...
class OctaneImager(bpy.types.Node, OctaneBaseNode):
    ...
    def init(self, context):
        ...
        self.inputs.new("OctaneImagerGroupDenoiser", OctaneImagerGroupDenoiser.bl_label).init()
        self.inputs.new("OctaneImagerDenoiser", OctaneImagerDenoiser.bl_label).init()
        self.inputs.new("OctaneImagerDenoiserType", OctaneImagerDenoiserType.bl_label).init()
        self.inputs.new("OctaneImagerDenoiseVolume", OctaneImagerDenoiseVolume.bl_label).init()
        self.inputs.new("OctaneImagerDenoisePrefilter", OctaneImagerDenoisePrefilter.bl_label).init()
...
_CLASSES=[
    ...
    OctaneImagerDenoiserType,
    OctaneImagerDenoiseVolume,
    OctaneImagerDenoisePrefilter,
