...
class OctaneCameraData(bpy.types.Node, OctaneBaseNode):
    ...
    max_distance: FloatProperty(name="Max Distance", default=100, min=0.001, max=10000000, soft_min=0.001,
                                soft_max=10000000, update=OctaneBaseNode.update_node_tree)
    ...
    def sync_custom_data(self, octane_node, octane_graph_node_data, depsgraph):
        ...
        if octane_name.endswith(self.VIEW_VECTOR_OUT):
            ...
            OctaneScriptNode.set_osl_pin(subnode, 0, "dmax", consts.SocketType.ST_FLOAT, consts.PinType.PT_FLOAT,
                                         consts.NodeType.NT_FLOAT, False, "", self.max_distance)
            ...
            OctaneScriptNode.set_osl_pin(subnode, 0, "dmax", consts.SocketType.ST_FLOAT, consts.PinType.PT_FLOAT,
                                         consts.NodeType.NT_FLOAT, False, "", self.max_z_depth)
            ...
            if self.keep_front_projection:
                ...
                is_environment_node = (getattr(octane_graph_node_data, "owner_type",
                                               consts.OctaneNodeTreeIDName.MATERIAL) == consts.OctaneNodeTreeIDName.WORLD)
                OctaneScriptNode.set_osl_pin(subnode, 0, "env_mode", consts.SocketType.ST_BOOL, consts.PinType.PT_BOOL,
                                             consts.NodeType.NT_BOOL, False, "", is_environment_node)
                OctaneScriptNode.set_osl_pin(subnode, 1, "blender_camera_mode", consts.SocketType.ST_BOOL,
                                             consts.PinType.PT_BOOL, consts.NodeType.NT_BOOL, True,
                                             consts.OCTANE_BLENDER_CAMERA_MODE, False)
                OctaneScriptNode.set_osl_pin(subnode, 2, "blender_camera_center_x", consts.SocketType.ST_FLOAT,
                                             consts.PinType.PT_FLOAT, consts.NodeType.NT_FLOAT, True,
                                             consts.OCTANE_BLENDER_CAMERA_CENTER_X, 0)
                OctaneScriptNode.set_osl_pin(subnode, 3, "blender_camera_center_y", consts.SocketType.ST_FLOAT,
                                             consts.PinType.PT_FLOAT, consts.NodeType.NT_FLOAT, True,
                                             consts.OCTANE_BLENDER_CAMERA_CENTER_Y, 0)
                OctaneScriptNode.set_osl_pin(subnode, 4, "blender_camera_region_x", consts.SocketType.ST_INT,
                                             consts.PinType.PT_INT, consts.NodeType.NT_INT, True,
                                             consts.OCTANE_BLENDER_CAMERA_REGION_WIDTH, 0)
                OctaneScriptNode.set_osl_pin(subnode, 5, "blender_camera_region_y", consts.SocketType.ST_INT,
                                             consts.PinType.PT_INT, consts.NodeType.NT_INT, True,
                                             consts.OCTANE_BLENDER_CAMERA_REGION_HEIGHT, 0)
            else:
                ...
                subnode.set_pin_id(consts.PinID.P_TRANSFORM, True,
                                   consts.OCTANE_BLENDER_STATIC_FRONT_PROJECTION_TRANSFORM, "")
    ...
    def load_custom_legacy_node(self, legacy_node, node_tree, context, report=None):
        if report is not None:
            if legacy_node.inputs["Max Z-Depth"].is_linked:
                report({"WARNING"},
                       "Outer links of the 'Max Z-Depth' socket are not supported in the Addon CameraData node anymore. So the link of the 'Max Z-Depth' socket in Node %s is discarded" % (
                           legacy_node.name))
            if legacy_node.inputs["Max Distance"].is_linked:
                report({"WARNING"},
                       "Outer links of the 'Max Distance' socket are not supported in the Addon CameraData node anymore. So the link of the 'Max Distance' socket in Node %s is discarded" % (
                           legacy_node.name))
            if legacy_node.inputs["Keep Front Projection"].is_linked:
                report({"WARNING"},
                       "Outer links of the 'Keep Front Projection' socket are not supported in the Addon CameraData node anymore. So the link of the 'Keep Front Projection' socket in Node %s is discarded" % (
                           legacy_node.name))
