...
MSGPACK_API_VERSION_NAMESPACE(v2)
{
  ...
  template<typename VisitorHolder> class context {
   public:
    context() : m_trail(0), m_cs(MSGPACK_CS_HEADER)
    {
    }
   ...
   private:
    template<typename T> static uint32_t next_cs(T p)
    {
      ...
      parse_return push(VisitorHolder &visitor_holder, msgpack_container_type type, uint32_t rest)
      {
        ...
        switch (type) {
          case MSGPACK_CT_ARRAY_ITEM:
            return visitor_holder.visitor().start_array_item() ? PARSE_CONTINUE :
                                                                 PARSE_STOP_VISITOR;
  ...
  inline parse_return context<VisitorHolder>::execute(const char *data,
                                                      std::size_t len,
                                                      std::size_t &off)
  {
    assert(len >= off);
    ...
    do {
      if (m_cs == MSGPACK_CS_HEADER) {
        ...
        if (0x00 <= selector && selector <= 0x7f) {  // Positive Fixnum
          ...
          parse_return ret = start_aggregate<fix_tag>(
              array_sv(holder()), array_ev(holder()), m_current, off);
          if (ret != PARSE_CONTINUE)
            return ret;
        }
        else if (0x80 <= selector && selector <= 0x8f) {  // FixMap
          parse_return ret = start_aggregate<fix_tag>(
              map_sv(holder()), map_ev(holder()), m_current, off);
          if (ret != PARSE_CONTINUE)
            return ret;
        }
      ...
      if (m_cs != MSGPACK_CS_HEADER || fixed_trail_again) {
        ...
        switch (m_cs) {
          ...
          case MSGPACK_CS_ARRAY_16: {
            parse_return ret = start_aggregate<uint16_t>(
                array_sv(holder()), array_ev(holder()), n, off);
            if (ret != PARSE_CONTINUE)
              return ret;

          } break;
          case MSGPACK_CS_ARRAY_32: {
            parse_return ret = start_aggregate<uint32_t>(
                array_sv(holder()), array_ev(holder()), n, off);
            if (ret != PARSE_CONTINUE)
              return ret;
          } break;
          case MSGPACK_CS_MAP_16: {
            parse_return ret = start_aggregate<uint16_t>(
                map_sv(holder()), map_ev(holder()), n, off);
            if (ret != PARSE_CONTINUE)
              return ret;
          } break;
  ...
  class parser : public detail::context<VisitorHolder> {
   ...
   public:
    ...
    /// Get buffer pointer.
    /**
     * You need to care about the memory is enable between buffer() and buffer() +
     * buffer_capacity() See:
     * https://github.com/msgpack/msgpack-c/wiki/v1_1_cpp_unpacker#msgpack-controls-a-buffer
     */
    char *buffer();
     ...
     *
     *
     * @return If one msgpack::object is unpacked, then return true, if msgpack::object is
     * incomplete and additional data is required, then return false. If data format is invalid,
     * throw msgpack::parse_error.
     *
     * See:
     * https://github.com/msgpack/msgpack-c/wiki/v1_1_cpp_unpacker#msgpack-controls-a-buffer
     */
  ...
  template<typename VisitorHolder, typename ReferencedBufferHook>
  inline parser<VisitorHolder, ReferencedBufferHook>
      &parser<VisitorHolder, ReferencedBufferHook>::operator=(this_type &&other)
  {
    this->~parser();
  ...
  template<typename Visitor> struct parse_helper : context<parse_helper<Visitor>> {
