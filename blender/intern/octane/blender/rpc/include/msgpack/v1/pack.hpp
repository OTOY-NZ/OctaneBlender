...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
  ...
  template<typename Stream> class packer {
   ...
   public:
    ...
    /**
     * The byte size of the packed data depends on `d`.
     * If `d` is zero or positive, the packed type is positive fixnum, uint8, uint16, uint32, or
     * uint64, else the packed type is negative fixnum, int8, int16, int32, or int64. The minimum
     * byte size expression is used. See
     ...
     * You need to call `pack_str_body(const char* b, uint32_t l)` after this function calling with
     * the same `l` value. See https://github.com/msgpack/msgpack/blob/master/spec.md#formats-str
     *
     ...
     * You need to call `pack_v4raw_body(const char* b, uint32_t l)` after this function calling
     * with the same `l` value. See
     ...
     * The minimum byte size length expression is used.
     * You need to call `pack_bin_body(const char* b, uint32_t l)` after this function calling with
     * the same `l` value. See
     ...
     * You need to call `pack_ext_body(const char* b, uint32_t l)` after this function calling with
     * the same `l` value. See https://github.com/msgpack/msgpack/blob/master/spec.md#formats-ext
     *
     * @param l The length of string.
     *
     * @return The reference of `*this`.
...
#  error msgpack-c supports only big endian and little endian
  ...
  template<typename Stream> inline packer<Stream>::packer(Stream * s) : m_stream(*s)
  {
  }
