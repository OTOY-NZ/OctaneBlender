...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
  ...
  template<typename T>
  typename msgpack::enable_if<!is_array<T>::value, msgpack::object const &>::type operator>>(
  ...
  template<typename T>
  typename msgpack::enable_if<!is_array<T>::value>::type operator<<(msgpack::object &o,
                                                                    T const &v);
  template<typename T, std::size_t N> void operator<<(msgpack::object &o, const T(&v)[N]);

  template<typename T>
  typename msgpack::enable_if<!is_array<T>::value>::type operator<<(msgpack::object::with_zone &o,
                                                                    T const &v);
  template<typename T, std::size_t N>
