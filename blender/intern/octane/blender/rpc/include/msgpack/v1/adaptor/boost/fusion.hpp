...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
  ...
  template<typename T> struct is_seq_no_pair_no_tuple {
    static bool const value = boost::fusion::traits::is_sequence<T>::value &&
                              !is_std_pair<T>::value
#if !defined(MSGPACK_USE_CPP03)
  ...
  struct as<
    ...
    static std::tuple<typename std::remove_reference<
        typename boost::fusion::result_of::at_c<T, Is>::type>::type...>
    to_tuple(U const &u, seq<Is...>)
    {
      return std::make_tuple(boost::fusion::at_c<Is>(u)...);
    }
    template<std::size_t... Is, typename U> static T to_t(U const &u, seq<Is...>)
    {
  ...
  struct object_with_zone<
      T,
      typename msgpack::enable_if<detail::is_seq_no_pair_no_tuple<T>::value>::type> {
