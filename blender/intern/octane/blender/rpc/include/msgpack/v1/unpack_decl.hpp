...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
   ...
   *
   * @return If the data should be referenced, then return true, otherwise (should be copied)
   * false.
   *
   * This function is called when unpacking STR, BIN, or EXT.
   *
   */
  typedef bool (*unpack_reference_func)(
      msgpack::type::object_type type, std::size_t size, void *user_data);
  ...
  class unpack_limit {
   public:
    unpack_limit(std::size_t array = 0xffffffff,
        ...
        : array_(array), map_(map), str_(str), bin_(bin), ext_(ext), depth_(depth)
    {
    }
  ...
  template<typename T>
  typename msgpack::enable_if<sizeof(T) == sizeof(fix_tag)>::type load(uint32_t &dst,
                                                                       const char *n);
   ...
   * @param referenced If the unpacked object contains reference of the buffer, then set as true,
   * otherwise false.
   ...
   * @param limit The size limit information of msgpack::object.
   *
   * This function is obsolete. Use the reference inteface version of unpack functions instead of
