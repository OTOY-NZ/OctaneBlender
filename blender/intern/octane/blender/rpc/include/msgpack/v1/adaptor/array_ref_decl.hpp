...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
  ...
  template<typename T>
  typename msgpack::enable_if<!msgpack::is_array<T const>::value, array_ref<T const>>::type
  make_array_ref(T const &t);

  template<typename T>
  typename msgpack::enable_if<!msgpack::is_array<T>::value, array_ref<T>>::type make_array_ref(
      T &t);
