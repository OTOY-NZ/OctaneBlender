...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
  ...
  struct convert<type::assoc_vector<K, V, Compare, Alloc>> {
    msgpack::object const &operator()(msgpack::object const &o,
                                      type::assoc_vector<K, V, Compare, Alloc> &v) const
    {
      if (o.type != msgpack::type::MAP) {
        throw msgpack::type_error();
      }
  ...
  struct pack<type::assoc_vector<K, V, Compare, Alloc>> {
                                        ...
                                        const type::assoc_vector<K, V, Compare, Alloc> &v) const
    {
      ...
      o.pack_map(size);
      for (typename type::assoc_vector<K, V, Compare, Alloc>::const_iterator it(v.begin()),
           it_end(v.end());
           it != it_end;
           ++it) {
        o.pack(it->first);
                    ...
                    const type::assoc_vector<K, V, Compare, Alloc> &v) const
    {
      ...
      if (v.empty()) {
        ...
        msgpack::object_kv *p = static_cast<msgpack::object_kv *>(o.zone.allocate_align(
            sizeof(msgpack::object_kv) * size, MSGPACK_ZONE_ALIGNOF(msgpack::object_kv)));
  ...
  struct pack<std::map<K, V, Compare, Alloc>> {
    template<typename Stream>
    msgpack::packer<Stream> &operator()(msgpack::packer<Stream> &o,
                                        const std::map<K, V, Compare, Alloc> &v) const
    {
      uint32_t size = checked_get_container_size(v.size());
      o.pack_map(size);
...
#  pragma GCC diagnostic pop
  ...
  struct convert<std::multimap<K, V, Compare, Alloc>> {
    msgpack::object const &operator()(msgpack::object const &o,
                                      std::multimap<K, V, Compare, Alloc> &v) const
    {
      if (o.type != msgpack::type::MAP) {
        throw msgpack::type_error();
      }
  ...
  struct pack<std::multimap<K, V, Compare, Alloc>> {
                                        ...
                                        const std::multimap<K, V, Compare, Alloc> &v) const
    {
      uint32_t size = checked_get_container_size(v.size());
      o.pack_map(size);
      for (typename std::multimap<K, V, Compare, Alloc>::const_iterator it(v.begin()),
           it_end(v.end());
           it != it_end;
           ++it) {
        o.pack(it->first);
