...
#define MSGPACK_DEFINE_MAP_IMPL(...) \
  MSGPACK_PP_SEQ_TO_TUPLE(MSGPACK_PP_SEQ_FOR_EACH( \
      MSGPACK_DEFINE_MAP_EACH_PROC, 0, MSGPACK_PP_VARIADIC_TO_SEQ(__VA_ARGS__)))

#define MSGPACK_DEFINE_MAP(...) \
  template<typename Packer> void msgpack_pack(Packer &pk) const \
  { \
    msgpack::type::make_define_map MSGPACK_DEFINE_MAP_IMPL(__VA_ARGS__).msgpack_pack(pk); \
  } \
  void msgpack_unpack(msgpack::object const &o) \
  { \
    msgpack::type::make_define_map MSGPACK_DEFINE_MAP_IMPL(__VA_ARGS__).msgpack_unpack(o); \
  } \
  template<typename MSGPACK_OBJECT> \
  void msgpack_object(MSGPACK_OBJECT *o, msgpack::zone &z) const \
  { \
    msgpack::type::make_define_map MSGPACK_DEFINE_MAP_IMPL(__VA_ARGS__).msgpack_object(o, z); \
  }
...
#define MSGPACK_ADD_ENUM(enum_name) \
  ...
  { \
    ...
    template<> struct object<enum_name> { \
      ...
      { \
        msgpack::underlying_type<enum_name>::type tmp = \
            static_cast<msgpack::underlying_type<enum_name>::type>(v); \
        msgpack::operator<<(o, tmp); \
      } \
    }; \
