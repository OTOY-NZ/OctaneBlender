...
#  define OTC_LOCAL_MEM_FENCE 0
#  define otc_barrier(flags) ((void)0)
 ...
 */
otc_device_inline float atomic_add_and_fetch_float(volatile otc_global float *source,
                                                   const float operand)
{
  union {
  ...
  } while (atomic_cmpxchg((volatile otc_global unsigned int *)source,
                          prev_value.int_value,
                          new_value.int_value) != prev_value.int_value);
  return new_value.float_value;
}

otc_device_inline float atomic_compare_and_swap_float(volatile otc_global float *dest,
                                                      const float old_val,
                                                      const float new_val)
{
  union {
  ...
  result.int_value = atomic_cmpxchg(
      (volatile otc_global unsigned int *)dest, prev_value.int_value, new_value.int_value);
...
#    define OTC_LOCAL_MEM_FENCE CLK_LOCAL_MEM_FENCE
#    define otc_barrier(flags) barrier(flags)
...
otc_device_inline float atomic_compare_and_swap_float(volatile float *dest,
                                                      const float old_val,
                                                      const float new_val)
{
  union {
...
#    define OTC_LOCAL_MEM_FENCE
#    define otc_barrier(flags) __syncthreads()
