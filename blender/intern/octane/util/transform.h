...
/* transform_point specialized for oct_global */
oct_device_inline float3 transform_point(oct_global const Transform *t, const float3 a)
{
  oct_global const float3x3 &b(*(oct_global const float3x3 *)t);
  return (a * b).xyz + make_float3(t->x.w, t->y.w, t->z.w);
}
#endif

oct_device_inline float3 transform_point(oct_private const Transform *t, const float3 a)
{
  ...
  oct_private const float3x3 &b(*(oct_private const float3x3 *)t);
  return (a * b).xyz + make_float3(t->x.w, t->y.w, t->z.w);
#else
...
oct_device_inline float3 transform_direction(oct_private const Transform *t, const float3 a)
{
#if defined(__KERNEL_SSE__) && defined(__KERNEL_SSE2__)
  const float4 aa(a.m128);
...
oct_device_inline float3 transform_direction_transposed(oct_private const Transform *t,
                                                        const float3 a)
{
...
oct_device_inline Transform make_transform(float a,
                                           float b,
                                           float c,
                                           float d,
                                           float e,
...
oct_device_inline Transform euler_to_transform(const float3 euler)
{
...
oct_device_inline Transform make_transform_frame(float3 N)
{
...
oct_device_inline Transform operator*(const Transform a, const Transform b)
{
...
oct_device_inline Transform transform_zero()
{
  Transform zero = {zero_float4(), zero_float4(), zero_float4()};
  return zero;
}

oct_device_inline void print_transform(const char *label, const Transform &t)
{
...
oct_device_inline Transform transform_translate(float3 t)
{
  return make_transform(1, 0, 0, t.x, 0, 1, 0, t.y, 0, 0, 1, t.z);
}

oct_device_inline Transform transform_translate(float x, float y, float z)
{
  return transform_translate(make_float3(x, y, z));
}

oct_device_inline Transform transform_scale(float3 s)
{
  return make_transform(s.x, 0, 0, 0, 0, s.y, 0, 0, 0, 0, s.z, 0);
}

oct_device_inline Transform transform_scale(float x, float y, float z)
{
  return transform_scale(make_float3(x, y, z));
}

oct_device_inline Transform transform_rotate(float angle, float3 axis)
{
  float s = sinf(angle);
  float c = cosf(angle);
  float t = 1.0f - c;
...
oct_device_inline Transform transform_euler(float3 euler)
{
...
oct_device_inline Transform transform_identity()
{
  return transform_scale(1.0f, 1.0f, 1.0f);
}

oct_device_inline bool operator==(const Transform &A, const Transform &B)
{
...
oct_device_inline bool transform_equal_threshold(const Transform &A,
                                                 const Transform &B,
...
oct_device_inline float3 transform_get_column(const Transform *t, int column)
{
  return make_float3(t->x[column], t->y[column], t->z[column]);
}

oct_device_inline void transform_set_column(Transform *t, int column, float3 value)
{
  t->x[column] = value.x;
  t->y[column] = value.y;
  t->z[column] = value.z;
...
oct_device_inline bool transform_uniform_scale(const Transform &tfm, float &scale)
{
...
oct_device_inline bool transform_negative_scale(const Transform &tfm)
{
...
oct_device_inline Transform transform_clear_scale(const Transform &tfm)
{
  Transform ntfm = tfm;
...
oct_device_inline Transform transform_empty()
{
...
oct_device_inline float4 quat_interpolate(float4 q1, float4 q2, float t)
{
...
oct_device_inline Transform transform_inverse(const Transform tfm)
{
...
oct_device_inline void transform_compose(oct_private Transform *tfm,
                                         oct_private const DecomposedTransform *decomp)
{
  /* rotation */
...
oct_device void transform_motion_array_interpolate(oct_private Transform *tfm,
                                                   oct_global const DecomposedTransform *motion,
                                                   uint numsteps,
                                                   float time)
{
  ...
  oct_global const DecomposedTransform *a = motion + step;
  oct_global const DecomposedTransform *b = motion + step + 1;
...
oct_device_inline bool transform_isfinite_safe(oct_private Transform *tfm)
{
  return isfinite_safe(tfm->x) && isfinite_safe(tfm->y) && isfinite_safe(tfm->z);
}

oct_device_inline bool transform_decomposed_isfinite_safe(oct_private DecomposedTransform *decomp)
{
  return isfinite_safe(decomp->x) && isfinite_safe(decomp->y) && isfinite_safe(decomp->z) &&
         isfinite_safe(decomp->w);
}

oct_device_inline Transform transform_euler(float3 euler, int mode)
{
...
oct_device_inline Transform make_transform(float3 translate, float3 euler, int mode, float3 scale)
{
...
oct_device_inline bool operator==(const DecomposedTransform &A, const DecomposedTransform &B)
{
