...
otc_device_inline bool local_index_to_coord(int4 rect, int idx, int *x, int *y)
{
  int w = rect.z - rect.x;
  *x = (idx % w) + rect.x;
  *y = (idx / w) + rect.y;
...
otc_device_inline int rect_size(int4 rect)
{
  return (rect.z - rect.x) * (rect.w - rect.y);
}
