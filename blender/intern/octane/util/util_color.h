...
otc_device uchar float_to_byte(float val)
{
  return ((val <= 0.0f) ? 0 :
                          ((val > (1.0f - 0.5f / 255.0f)) ? 255 : (uchar)((255.0f * val) + 0.5f)));
}

otc_device uchar4 color_float_to_byte(float3 c)
{
  uchar r, g, b;
...
otc_device_inline float3 color_byte_to_float(uchar4 c)
{
  return make_float3(c.x * (1.0f / 255.0f), c.y * (1.0f / 255.0f), c.z * (1.0f / 255.0f));
}

otc_device float color_srgb_to_linear(float c)
{
  if (c < 0.04045f)
...
otc_device float color_linear_to_srgb(float c)
{
  if (c < 0.0031308f)
...
otc_device float3 rgb_to_hsv(float3 rgb)
{
...
otc_device float3 hsv_to_rgb(float3 hsv)
{
...
otc_device float3 xyY_to_xyz(float x, float y, float Y)
{
  float X, Z;
 ...
 * This method gives a constant bias, which can be easily compensated by multiplication with
 * bias_coeff. Gives better results for exponents near 1 (e. g. 4/5). exp = exponent, encoded as
 * uint32_t e2coeff = 2^(127/exponent - 127) * bias_coeff^(1/exponent), encoded as uint32_t
 */
template<unsigned exp, unsigned e2coeff> otc_device_inline ssef fastpow(const ssef &arg)
{
  ssef ret;
...
otc_device_inline ssef improve_5throot_solution(const ssef &old_result, const ssef &x)
{
...
otc_device_inline ssef fastpow24(const ssef &arg)
{
...
otc_device ssef color_srgb_to_linear(const ssef &c)
{
...
otc_device float3 color_srgb_to_linear_v3(float3 c)
{
  return make_float3(
      color_srgb_to_linear(c.x), color_srgb_to_linear(c.y), color_srgb_to_linear(c.z));
}

otc_device float3 color_linear_to_srgb_v3(float3 c)
{
  return make_float3(
      color_linear_to_srgb(c.x), color_linear_to_srgb(c.y), color_linear_to_srgb(c.z));
}

otc_device float4 color_srgb_to_linear_v4(float4 c)
{
#ifdef __KERNEL_SSE2__
  ssef r_ssef;
