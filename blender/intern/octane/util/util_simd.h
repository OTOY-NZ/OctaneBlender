...
} inf otc_maybe_unused, pos_inf otc_maybe_unused;
...
otc_device_inline int bitscan(int value)
{
  assert(value != 0);
  int bit = 0;
  while ((value & (1 << bit)) == 0) {
...
otc_device_inline int __bsr(int value)
{
  assert(value != 0);
  int bit = 0;
  while (value >>= 1) {
