...
{
  if (getenv("OCTANE_HIP_ADAPTIVE_COMPILE") != NULL)
    adaptive_compile = true;
}

void DebugFlags::Metal::reset()
{
  if (getenv("OCTANE_METAL_ADAPTIVE_COMPILE") != NULL)
    adaptive_compile = true;

  if (auto str = getenv("OCTANE_METAL_LOCAL_ATOMIC_SORT"))
    use_local_atomic_sort = (atoi(str) != 0);

  if (auto str = getenv("OCTANE_METAL_NANOVDB"))
    use_nanovdb = (atoi(str) != 0);

  if (auto str = getenv("OCTANE_METAL_ASYNC_PSO_CREATION"))
