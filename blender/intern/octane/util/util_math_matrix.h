...
otc_device_inline void math_vector_zero(float *v, int n)
{
  for (int i = 0; i < n; i++) {
    v[i] = 0.0f;
  }
...
otc_device_inline void math_matrix_zero(float *A, int n)
{
...
otc_device_inline void math_vector_add(float *a, const float *otc_restrict b, int n)
{
  for (int i = 0; i < n; i++) {
    a[i] += b[i];
  }
...
otc_device_inline void math_vector_mul(float *a, const float *otc_restrict b, int n)
{
  for (int i = 0; i < n; i++) {
    a[i] *= b[i];
  }
...
otc_device_inline void math_vector_mul_strided(otc_global float *a,
                                               const float *otc_restrict b,
                                               int astride,
                                               int n)
{
  for (int i = 0; i < n; i++) {
...
otc_device_inline void math_vector_scale(float *a, float b, int n)
{
  for (int i = 0; i < n; i++) {
    a[i] *= b;
  }
...
otc_device_inline void math_vector_max(float *a, const float *otc_restrict b, int n)
{
  for (int i = 0; i < n; i++) {
    a[i] = max(a[i], b[i]);
  }
...
otc_device_inline void math_vec3_add(float3 *v, int n, float *x, float3 w)
{
  for (int i = 0; i < n; i++) {
    v[i] += w * x[i];
  }
...
otc_device_inline void math_vec3_add_strided(
    otc_global float3 *v, int n, float *x, float3 w, int stride)
{
  for (int i = 0; i < n; i++) {
    otc_global float *elem = (otc_global float *)(v + i * stride);
 ...
 * Note: TriMatrix refers to a square matrix that is symmetric, and therefore its upper-triangular
 * part isn't stored. */

otc_device_inline void math_trimatrix_add_diagonal(otc_global float *A,
                                                   int n,
                                                   float val,
                                                   int stride)
{
...
otc_device_inline void math_matrix_add_gramian(float *A,
                                               int n,
                                               const float *otc_restrict v,
                                               float weight)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
...
/* Add Gramian matrix of v to A.
 * The Gramian matrix of v is vt*v, so element (i,j) is v[i]*v[j]. */
otc_device_inline void math_trimatrix_add_gramian_strided(
    otc_global float *A, int n, const float *otc_restrict v, float weight, int stride)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
...
otc_device_inline void math_trimatrix_add_gramian(otc_global float *A,
                                                  int n,
                                                  const float *otc_restrict v,
                                                  float weight)
{
...
otc_device_inline void math_matrix_transpose(otc_global float *A, int n, int stride)
{
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < i; j++) {
...
otc_device void math_trimatrix_cholesky(otc_global float *A, int n, int stride)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
...
/* Solve A*S=y for S given A and y, where A is symmetrical positive-semidefinite and both inputs
 ...
 * symmetrical positive-semidefinite by construction, so we can just use this function with
 * A=Xt*W*X and y=Xt*W*y. */
otc_device_inline void math_trimatrix_vec3_solve(otc_global float *A,
                                                 otc_global float3 *y,
                                                 int n,
                                                 int stride)
{
  /* Since the first entry of the design row is always 1, the upper-left element of XtWX is a good
   * heuristic for the amount of pixels considered (with weighting), therefore the amount of
   * correction is scaled based on it. */
 ...
 */
otc_device void math_matrix_jacobi_eigendecomposition(float *A,
                                                      otc_global float *V,
                                                      int n,
                                                      int v_stride)
{
  ...
  for (int sweep = 0; sweep < 8; sweep++) {
    ...
    for (int row = 1; row < n; row++) {
      for (int col = 0; col < row; col++) {
        ...
        /* If we're in a later sweep and the element already is very small, just set it to zero and
         * skip the rotation. */
        ...
        /* If we're in one of the first sweeps and the element is smaller than the threshold, skip
         * it. */
        if (sweep < 3 && (abs_element < threshold)) {
          continue;
        }

        /* Determine rotation: The rotation is characterized by its angle phi - or, in the actual
         * implementation, sin(phi) and cos(phi). To find those, we first compute their ratio -
         * that might be unstable if the angle approaches 90°, so there's a fallback for that case.
...
otc_device_inline void math_vector_zero_sse(float4 *A, int n)
{
  for (int i = 0; i < n; i++) {
...
otc_device_inline void math_matrix_zero_sse(float4 *A, int n)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
...
otc_device_inline void math_matrix_add_gramian_sse(float4 *A,
                                                   int n,
                                                   const float4 *otc_restrict v,
                                                   float4 weight)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
...
otc_device_inline void math_vector_add_sse(float4 *V, int n, const float4 *otc_restrict a)
{
  for (int i = 0; i < n; i++) {
    V[i] += a[i];
  }
...
otc_device_inline void math_vector_mul_sse(float4 *V, int n, const float4 *otc_restrict a)
{
  for (int i = 0; i < n; i++) {
    V[i] *= a[i];
  }
...
otc_device_inline void math_vector_max_sse(float4 *a, const float4 *otc_restrict b, int n)
{
  for (int i = 0; i < n; i++) {
    a[i] = max(a[i], b[i]);
  }
...
otc_device_inline void math_matrix_hsum(float *A, int n, const float4 *otc_restrict B)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
