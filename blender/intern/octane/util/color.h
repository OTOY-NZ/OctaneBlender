...
oct_device uchar float_to_byte(float val)
{
  return ((val <= 0.0f) ? 0 :
                          ((val > (1.0f - 0.5f / 255.0f)) ? 255 : (uchar)((255.0f * val) + 0.5f)));
}

oct_device float byte_to_float(uchar val)
{
  return val * (1.0f / 255.0f);
}

oct_device uchar4 color_float_to_byte(float3 c)
{
  uchar r, g, b;
...
oct_device uchar4 color_float4_to_uchar4(float4 c)
{
  uchar r, g, b, a;
...
oct_device_inline float3 color_byte_to_float(uchar4 c)
{
  return make_float3(c.x * (1.0f / 255.0f), c.y * (1.0f / 255.0f), c.z * (1.0f / 255.0f));
}

oct_device_inline float4 color_uchar4_to_float4(uchar4 c)
{
  return make_float4(
      c.x * (1.0f / 255.0f), c.y * (1.0f / 255.0f), c.z * (1.0f / 255.0f), c.w * (1.0f / 255.0f));
}

oct_device float color_srgb_to_linear(float c)
{
  if (c < 0.04045f)
...
oct_device float color_linear_to_srgb(float c)
{
  if (c < 0.0031308f)
...
oct_device float3 rgb_to_hsv(float3 rgb)
{
...
oct_device float3 hsv_to_rgb(float3 hsv)
{
...
oct_device float3 rgb_to_hsl(float3 rgb)
{
...
oct_device float3 hsl_to_rgb(float3 hsl)
{
...
oct_device float3 xyY_to_xyz(float x, float y, float Y)
{
  float X, Z;
 ...
 */
template<unsigned exp, unsigned e2coeff> oct_device_inline float4 fastpow(const float4 &arg)
{
...
oct_device_inline float4 improve_5throot_solution(const float4 &old_result, const float4 &x)
{
...
oct_device_inline float4 fastpow24(const float4 &arg)
{
...
oct_device float4 color_srgb_to_linear(const float4 &c)
{
...
oct_device float3 color_highlight_compress(float3 color, oct_private float3 *variance)
{
  color += one_float3();
  if (variance) {
...
oct_device float3 color_highlight_uncompress(float3 color)
{
