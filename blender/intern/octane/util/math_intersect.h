...
oct_device bool ray_sphere_intersect(float3 ray_P,
                                     float3 ray_D,
                                     float ray_tmin,
                                     float ray_tmax,
                                     float3 sphere_P,
                                     float sphere_radius,
                                     oct_private float3 *isect_P,
                                     oct_private float *isect_t)
{
...
oct_device bool ray_aligned_disk_intersect(float3 ray_P,
                                           float3 ray_D,
                                           float ray_tmin,
                                           float ray_tmax,
                                           float3 disk_P,
...
oct_device bool ray_disk_intersect(float3 ray_P,
                                   float3 ray_D,
                                   float ray_tmin,
                                   float ray_tmax,
                                   float3 disk_P,
...
oct_device_forceinline float ray_triangle_rcp(const float x)
{
...
oct_device_inline float ray_triangle_dot(const float3 a, const float3 b)
{
...
oct_device_inline float3 ray_triangle_cross(const float3 a, const float3 b)
{
#if defined(__KERNEL_SSE41__) && defined(__KERNEL_SSE__)
  return make_float3(
...
oct_device_forceinline bool ray_triangle_intersect(const float3 ray_P,
                                                   const float3 ray_D,
                                                   ...
                                                   const float3 tri_b,
                                                   const float3 tri_c,
                                                   oct_private float *isect_u,
                                                   oct_private float *isect_v,
                                                   oct_private float *isect_t)
{
...
oct_device_forceinline bool ray_triangle_intersect_self(const float3 ray_P,
                                                        const float3 ray_D,
 ...
 * quad are rejected.
 */
oct_device bool ray_quad_intersect(float3 ray_P,
                                   float3 ray_D,
                                   float ray_tmin,
                                   float ray_tmax,
                                   float3 quad_P,
