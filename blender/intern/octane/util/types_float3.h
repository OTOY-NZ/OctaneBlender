...
struct float3
#  else
struct oct_try_align(16) float3
#  endif
{
...
oct_device_inline float3 make_float3(float x, float y, float z);
#endif /* __KERNEL_NATIVE_VECTOR_TYPES__ */

oct_device_inline float3 make_float3(float f);
oct_device_inline void print_float3(oct_private const char *label, const float3 a);
...
struct packed_float3 {
  oct_device_inline_method packed_float3(){};

  oct_device_inline_method packed_float3(const float3 &a) : x(a.x), y(a.y), z(a.z) {}

  oct_device_inline_method operator float3() const
  {
    return make_float3(x, y, z);
  }

  oct_device_inline_method packed_float3 &operator=(const float3 &a)
  {
    x = a.x;
    y = a.y;
    z = a.z;
