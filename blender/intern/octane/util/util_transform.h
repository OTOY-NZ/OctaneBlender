...
otc_device_inline float3 transform_point(const Transform *t, const float3 a)
{
...
otc_device_inline float3 transform_direction(const Transform *t, const float3 a)
{
#if defined(__KERNEL_SSE__) && defined(__KERNEL_SSE2__)
  ssef x, y, z, w, aa;
  aa = a.m128;
...
otc_device_inline float3 transform_direction_transposed(const Transform *t, const float3 a)
{
...
otc_device_inline Transform make_transform(float a,
                                           float b,
                                           float c,
                                           float d,
                                           float e,
...
otc_device_inline Transform euler_to_transform(const float3 euler)
{
...
otc_device_inline Transform make_transform_frame(float3 N)
{
...
otc_device_inline Transform operator*(const Transform a, const Transform b)
{
...
otc_device_inline void print_transform(const char *label, const Transform &t)
{
...
otc_device_inline Transform transform_translate(float3 t)
{
  return make_transform(1, 0, 0, t.x, 0, 1, 0, t.y, 0, 0, 1, t.z);
}

otc_device_inline Transform transform_translate(float x, float y, float z)
{
  return transform_translate(make_float3(x, y, z));
}

otc_device_inline Transform transform_scale(float3 s)
{
  return make_transform(s.x, 0, 0, 0, 0, s.y, 0, 0, 0, 0, s.z, 0);
}

otc_device_inline Transform transform_scale(float x, float y, float z)
{
  return transform_scale(make_float3(x, y, z));
}

otc_device_inline Transform transform_rotate(float angle, float3 axis)
{
  float s = sinf(angle);
  float c = cosf(angle);
  float t = 1.0f - c;
...
otc_device_inline Transform transform_euler(float3 euler)
{
...
otc_device_inline float3 transform_get_column(const Transform *t, int column)
{
  return make_float3(t->x[column], t->y[column], t->z[column]);
}

otc_device_inline void transform_set_column(Transform *t, int column, float3 value)
{
  t->x[column] = value.x;
  t->y[column] = value.y;
  t->z[column] = value.z;
...
otc_device_inline bool transform_uniform_scale(const Transform &tfm, float &scale)
{
...
otc_device_inline bool transform_negative_scale(const Transform &tfm)
{
...
otc_device_inline Transform transform_clear_scale(const Transform &tfm)
{
  Transform ntfm = tfm;
...
otc_device_inline Transform transform_empty()
{
...
otc_device_inline float4 quat_interpolate(float4 q1, float4 q2, float t)
{
...
otc_device_inline Transform transform_quick_inverse(Transform M)
{
...
otc_device_inline void transform_compose(Transform *tfm, const DecomposedTransform *decomp)
{
  /* rotation */
...
otc_device void transform_motion_array_interpolate(Transform *tfm,
                                                   const otc_global DecomposedTransform *motion,
                                                   uint numsteps,
                                                   float time)
{
  ...
  const otc_global DecomposedTransform *a = motion + step;
  const otc_global DecomposedTransform *b = motion + step + 1;
...
#  ifdef WITH_EMBREE
otc_device void transform_motion_array_interpolate_straight(
    Transform *tfm, const otc_global DecomposedTransform *motion, uint numsteps, float time)
{
  /* Figure out which steps we need to interpolate. */
  int maxstep = numsteps - 1;
  int step = min((int)(time * maxstep), maxstep - 1);
...
otc_device_inline bool operator==(const DecomposedTransform &A, const DecomposedTransform &B)
{
...
#  define OPENCL_TRANSFORM_ADDRSPACE_DECLARE(function) \
    otc_device_inline float3 OPENCL_TRANSFORM_ADDRSPACE_GLUE(function, _addrspace)( \
        otc_addr_space const Transform *t, const float3 a) \
    { \
