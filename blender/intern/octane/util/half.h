...
oct_device_inline float half_to_float(half h_in)
{
  float f;
  union {
    half h;
  ...
  *((oct_private int *)&f) = ((val.s & 0x8000) << 16) | (((val.s & 0x7c00) + 0x1C000) << 13) |
                             ((val.s & 0x03FF) << 13);
...
oct_device_inline half float_to_half_image(float f)
{
...
oct_device_inline float half_to_float_image(half h)
{
#if defined(__KERNEL_METAL__)
...
oct_device_inline float4 half4_to_float4_image(const half4 h)
{
...
oct_device_inline half float_to_half_display(const float f)
{
#if defined(__KERNEL_METAL__) || defined(__KERNEL_ONEAPI__)
  return half(min(f, 65504.0f));
#elif defined(__KERNEL_CUDA__) || defined(__KERNEL_HIP__)
...
oct_device_inline half4 float4_to_half4_display(const float4 f)
{
