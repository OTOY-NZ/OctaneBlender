...
oct_device_inline void math_vector_zero(oct_private float *v, int n)
{
  for (int i = 0; i < n; i++) {
    v[i] = 0.0f;
  }
...
oct_device_inline void math_matrix_zero(oct_private float *A, int n)
{
...
oct_device_inline void math_vector_add(oct_private float *a,
                                       oct_private const float *oct_restrict b,
                                       int n)
{
  for (int i = 0; i < n; i++) {
    a[i] += b[i];
...
oct_device_inline void math_vector_mul(oct_private float *a,
                                       oct_private const float *oct_restrict b,
                                       int n)
{
  for (int i = 0; i < n; i++) {
...
oct_device_inline void math_vector_mul_strided(oct_global float *a,
                                               oct_private const float *oct_restrict b,
                                               int astride,
                                               int n)
{
...
oct_device_inline void math_vector_scale(oct_private float *a, float b, int n)
{
  for (int i = 0; i < n; i++) {
    a[i] *= b;
  }
...
oct_device_inline void math_vector_max(oct_private float *a,
                                       oct_private const float *oct_restrict b,
                                       int n)
{
  for (int i = 0; i < n; i++) {
...
oct_device_inline void math_vec3_add(oct_private float3 *v, int n, oct_private float *x, float3 w)
{
  for (int i = 0; i < n; i++) {
    v[i] += w * x[i];
  }
...
oct_device_inline void math_vec3_add_strided(
    oct_global float3 *v, int n, oct_private float *x, float3 w, int stride)
{
  for (int i = 0; i < n; i++) {
    oct_global float *elem = (oct_global float *)(v + i * stride);
...
oct_device_inline void math_trimatrix_add_diagonal(oct_global float *A,
                                                   int n,
                                                   float val,
                                                   int stride)
{
...
oct_device_inline void math_matrix_add_gramian(oct_private float *A,
                                               int n,
                                               oct_private const float *oct_restrict v,
                                               float weight)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
...
/* Add Gramian matrix of v to A.
 * The Gramian matrix of v is vt*v, so element (i,j) is v[i]*v[j]. */
oct_device_inline void math_trimatrix_add_gramian_strided(
    oct_global float *A, int n, oct_private const float *oct_restrict v, float weight, int stride)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
...
oct_device_inline void math_trimatrix_add_gramian(oct_global float *A,
                                                  int n,
                                                  oct_private const float *oct_restrict v,
                                                  float weight)
{
...
oct_device_inline void math_matrix_transpose(oct_global float *A, int n, int stride)
{
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < i; j++) {
...
oct_device void math_trimatrix_cholesky(oct_global float *A, int n, int stride)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
...
oct_device_inline void math_trimatrix_vec3_solve(oct_global float *A,
                                                 oct_global float3 *y,
                                                 int n,
                                                 int stride)
{
 ...
 */
oct_device void math_matrix_jacobi_eigendecomposition(oct_private float *A,
                                                      oct_global float *V,
                                                      int n,
                                                      int v_stride)
{
...
oct_device_inline void math_vector_zero_sse(float4 *A, int n)
{
  for (int i = 0; i < n; i++) {
    A[i] = zero_float4();
  }
...
oct_device_inline void math_matrix_zero_sse(float4 *A, int n)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
...
oct_device_inline void math_matrix_add_gramian_sse(float4 *A,
                                                   int n,
                                                   const float4 *oct_restrict v,
                                                   float4 weight)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
...
oct_device_inline void math_vector_add_sse(float4 *V, int n, const float4 *oct_restrict a)
{
  for (int i = 0; i < n; i++) {
    V[i] += a[i];
  }
...
oct_device_inline void math_vector_mul_sse(float4 *V, int n, const float4 *oct_restrict a)
{
  for (int i = 0; i < n; i++) {
    V[i] *= a[i];
  }
...
oct_device_inline void math_vector_max_sse(float4 *a, const float4 *oct_restrict b, int n)
{
  for (int i = 0; i < n; i++) {
    a[i] = max(a[i], b[i]);
  }
...
oct_device_inline void math_matrix_hsum(float *A, int n, const float4 *oct_restrict B)
{
  for (int row = 0; row < n; row++) {
    for (int col = 0; col <= row; col++) {
