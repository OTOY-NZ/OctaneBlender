...
oct_device_inline size_t align_up(size_t offset, size_t alignment)
{
  return (offset + alignment - 1) & ~(alignment - 1);
}

oct_device_inline size_t divide_up(size_t x, size_t y)
{
  return (x + y - 1) / y;
}

oct_device_inline size_t round_up(size_t x, size_t multiple)
{
  return ((x + multiple - 1) / multiple) * multiple;
}

oct_device_inline size_t round_down(size_t x, size_t multiple)
{
  return (x / multiple) * multiple;
}

oct_device_inline bool is_power_of_two(size_t x)
{
  return (x & (x - 1)) == 0;
}
...
oct_device_inline void print_float(oct_private const char *label, const float a)
{
