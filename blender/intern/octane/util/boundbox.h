...
class BoundBox {
 public:
  ...
  __forceinline void grow(const float3 &pt, float border)
  {
    ...
    min = oct::min(pt - shift, min);
    max = oct::max(pt + shift, max);
  }

  __forceinline void grow(const BoundBox &bbox)
  {
    min = oct::min(bbox.min, min);
    max = oct::max(bbox.max, max);
  }
  ...
  {
    min = oct::max(min, bbox.min);
    max = oct::min(max, bbox.max);
  }
...
class BoundBox2D {
 public:
  ...
  BoundBox2D clamp(float mn = 0.0f, float mx = 1.0f)
  {
    ...
    result.left = oct::clamp(left, mn, mx);
    result.right = oct::clamp(right, mn, mx);
    result.bottom = oct::clamp(bottom, mn, mx);
    result.top = oct::clamp(top, mn, mx);
